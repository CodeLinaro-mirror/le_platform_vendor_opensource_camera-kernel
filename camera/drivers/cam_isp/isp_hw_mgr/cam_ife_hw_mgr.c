...
struct cam_ife_hw_mgr g_ife_hw_mgr;
struct cam_ife_hw_mgr_sensor_grp_cfg  g_ife_sns_grp_cfg;
...
static uint32_t max_sfe_out_res;
	...
	uint64_t request_id, struct cam_isp_prepare_hw_update_data *prepare_hw_data)
{
	...
	if (debug_drv)
		CAM_INFO(CAM_PERF,
			...
			c_ctx->ctx_index, CAM_BOOL_TO_YESNO(drv_config->drv_en),
	...
	CAM_DBG(CAM_PERF,
		...
		prepare_hw_data->packet_opcode_type, request_id,
		CAM_BOOL_TO_YESNO(g_ife_hw_mgr.debug_cfg.disable_isp_drv),
		c_ctx->ctx_index, drv_config->drv_en, drv_config->path_idle_en,
	...
	if (drv_config->drv_en)
		c_ctx->drv_path_idle_en = drv_config->path_idle_en;

	for (i = 0; i < c_ctx->num_base; i++) {
		if (c_ctx->base[i].hw_type != CAM_ISP_HW_TYPE_CSID)
			continue;

		hw_intf = ife_hw_mgr->csid_devices[c_ctx->base[i].idx];
		if (hw_intf && hw_intf->hw_ops.process_cmd) {
			...
			if (rc) {
				CAM_ERR(CAM_PERF,
					...
					request_id, i, c_ctx->base[i].idx, rc, c_ctx->ctx_index);
				break;
			}
		}
	}
...
bool cam_isp_is_ctx_primary_rdi(struct cam_ife_hw_mgr_ctx  *ctx)
{
	...
	if (c_ctx->flags.is_rdi_only_context || c_ctx->flags.rdi_pd_context)
		return true;

	return false;
}

int cam_ife_mgr_finish_clk_bw_update(
	struct cam_ife_hw_mgr_ctx *ctx,
	uint64_t request_id, bool skip_clk_data_rst)
{
	...
	clk_bw_args.is_drv_config_en = (bool) (c_ctx->drv_path_idle_en & CAM_ISP_PXL_PATH);

	for (i = 0; i < c_ctx->num_base; i++) {
		clk_bw_args.hw_intf = NULL;
		clk_bw_args.clock_updated = false;
		CAM_DBG(CAM_PERF,
			"Clock/BW Update for ctx:%u req_id:%u i:%d num_vfe_out:%d num_sfe_out:%d in_rd:%d",
			c_ctx->ctx_index,request_id, i, c_ctx->num_acq_vfe_out,
			c_ctx->num_acq_sfe_out, !list_empty(&c_ctx->res_list_ife_in_rd));
		if ((c_ctx->base[i].hw_type == CAM_ISP_HW_TYPE_VFE) &&
			(c_ctx->num_acq_vfe_out || (!list_empty(&c_ctx->res_list_ife_in_rd))))
			clk_bw_args.hw_intf = g_ife_hw_mgr.ife_devices[c_ctx->base[i].idx]->hw_intf;
		else if ((c_ctx->base[i].hw_type == CAM_ISP_HW_TYPE_SFE) &&
			(c_ctx->num_acq_sfe_out || (!list_empty(&c_ctx->res_list_ife_in_rd))))
			clk_bw_args.hw_intf = g_ife_hw_mgr.sfe_devices[c_ctx->base[i].idx]->hw_intf;
		else
			continue;

		CAM_DBG(CAM_PERF,
			"Apply Clock/BW for ctx:%u req:%d i:%d hw_idx=%d hw_type:%d inline:%s num_vfe_out:%d num_sfe_out:%d in_rd:%d",
			c_ctx->ctx_index, request_id, i, clk_bw_args.hw_intf->hw_idx,
			clk_bw_args.hw_intf->hw_type,
			CAM_BOOL_TO_YESNO(clk_bw_args.is_drv_config_en),
			c_ctx->num_acq_vfe_out, c_ctx->num_acq_sfe_out,
			!list_empty(&c_ctx->res_list_ife_in_rd));
		...
		if (rc) {
			CAM_ERR(CAM_PERF,
				...
				c_ctx->ctx_index, request_id, i, c_ctx->base[i].idx,
				c_ctx->base[i].hw_type, rc);
			break;
		}
	...
	for (i = 0; i < c_ctx->num_base; i++) {
		...
		if (hw_if->hw_ops.process_cmd) {
			...
			CAM_DBG(CAM_ISP,
				...
				overflow_info.is_bus_overflow, c_ctx->ctx_index);
	...
	bool     start_only)
{
	...
	for (j = c_ctx->num_base - 1 ; j >= 0; j--) {
		...
		if (cnt) {
			...
			start_args.is_drv_config_en = (bool) c_ctx->drv_path_idle_en;
...
void cam_ife_hw_mgr_stop_hw_res(
	struct cam_isp_hw_mgr_res   *isp_hw_res)
{
	int i;
	struct cam_hw_intf      *hw_intf;
	...
	struct cam_ife_hw_mgr_ctx *ctx)
{
	...
	if (!c_ctx->flags.init_done) {
		CAM_WARN(CAM_ISP, "ctx is not in init state, ctx_idx: %u", c_ctx->ctx_index);
		return;
	}
	...
	if (c_ctx->ctx_type == CAM_IFE_CTX_TYPE_SFE) {
		...
		/* Deinit SFE OUT */
		for (i = 0; i < c_ctx->num_acq_sfe_out; i++)
	...
	struct cam_ife_hw_mgr_ctx *ctx)
{
	...
	/* INIT IFE OUT */
	CAM_DBG(CAM_ISP, "INIT IFE OUT RESOURCES in ctx id:%u num_acq_vfe_out :%d",
		c_ctx->ctx_index, c_ctx->num_acq_vfe_out);
...
bool cam_ife_hw_mgr_check_path_port_compat(
	...
	struct cam_ife_hw_mgr_ctx    *hwr_mgr_ctx)
{
	...
	for (i = 0; i < c_ctx->num_acq_sfe_out; i++) {
		for (j = 0; j < CAM_ISP_HW_SPLIT_MAX; j++) {
			hw_mgr_res = &c_ctx->res_list_sfe_out[i];
			hw_res = hw_mgr_res->hw_res[j];
			if (hw_res && hw_res->hw_intf)
				CAM_INFO(CAM_ISP,
...
static void cam_ife_hw_mgr_dump_acq_rsrc_for_all_ctx(
		struct cam_ife_hw_mgr_ctx *hw_mgr_ctx)
{
	...
	list_for_each_entry_safe(c_ctx, ctx_temp,
		&g_ife_hw_mgr.used_ctx_list, list) {
		CAM_INFO_RATE_LIMIT(CAM_ISP,
			"ctx id:%u is_dual:%d num_base:%d rdi only:%d",
			c_ctx->ctx_index, c_ctx->flags.is_dual,
			c_ctx->num_base, c_ctx->flags.is_rdi_only_context);

		cam_ife_hw_mgr_dump_acquire_resources(hw_mgr_ctx);
	}
	mutex_unlock(&g_ife_hw_mgr.ctx_mutex);
}
	...
	uint32_t num_pd_port, uint32_t num_rdi_port, int acquire_failed)
{
	...
	CAM_INFO(CAM_ISP,
		...
		c_ctx->ctx_index, c_ctx->flags.per_port_en);

	return;

fail:
	CAM_ERR(CAM_ISP,
		"Failed to acquire %s-IFE/SFE with [%u pix] [%u pd] [%u rdi] ports for ctx_idx:%u per_port_enabled :%d",
		(c_ctx->flags.is_dual) ? "dual" : "single",
		num_pix_port, num_pd_port, num_rdi_port, c_ctx->ctx_index,
		c_ctx->flags.per_port_en);
	...
	if (c_ctx->is_hw_ctx_acq)
		CAM_INFO(CAM_ISP, "HW_CTXT [SRC:DST_MASK] [%d:0x%x] [%d:0x%x] [%d:0x%x]",
			...
			c_ctx->acq_hw_ctxt_src_dst_map[CAM_ISP_MULTI_CTXT_0],
			CAM_ISP_MULTI_CTXT_1,
			c_ctx->acq_hw_ctxt_src_dst_map[CAM_ISP_MULTI_CTXT_1],
			CAM_ISP_MULTI_CTXT_2,
			c_ctx->acq_hw_ctxt_src_dst_map[CAM_ISP_MULTI_CTXT_2]);

	cam_ife_hw_mgr_dump_acq_rsrc_for_all_ctx(hw_mgr_ctx);
}
		...
		uint32_t  base_idx, uint32_t stop_cmd)
{
	...
	list_for_each_entry(hw_mgr_res, stop_list, list) {
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			if (!hw_mgr_res->hw_res[i] ||
				(hw_mgr_res->hw_res[i]->res_state !=
				CAM_ISP_RESOURCE_STATE_STREAMING && !c_ctx->flags.per_port_en))
				continue;
			...
			CAM_DBG(CAM_ISP, "base_idx %d ctx_idx: %u res:%s res_id %d cnt %u",
				base_idx, c_ctx->ctx_index, isp_res->res_name,
				isp_res->res_id, cnt);
	...
	int index)
{
	...
	if (c_ctx->flags.per_port_en && (index != CAM_IFE_STREAM_GRP_INDEX_NONE)) {
		ife_src_list_head =
		...
		num_out = max_ife_out_res;
		CAM_DBG(CAM_ISP, "entered per_port CTX:%d", c_ctx->ctx_index);
	} else {
		ife_src_list_head = &c_ctx->res_list_ife_src;
		csid_res_list_head = &c_ctx->res_list_ife_csid;
		free_res_list_head = &c_ctx->free_res_list;
		num_out = c_ctx->num_acq_vfe_out;
		CAM_DBG(CAM_ISP, "entered per_port disable CTX:%d", c_ctx->ctx_index);
	}
	...
	if (c_ctx->ctx_type == CAM_IFE_CTX_TYPE_SFE) {
		/* sfe leaf resource */
		num_out = c_ctx->num_acq_sfe_out;
		for (i = 0; i < num_out; i++) {
			cam_ife_hw_mgr_free_hw_res(
				&c_ctx->res_list_sfe_out[i], false);
	...
	enum cam_isp_hw_type             hw_type)
{
	...
	if (!c_ctx->num_base) {
		...
		CAM_DBG(CAM_ISP,
			...
			split_id, c_ctx->ctx_index, base_idx, c_ctx->num_base, hw_type);
	} else {
	...
	struct cam_ife_hw_mgr_ctx        *ctx)
{
	...
	list_for_each_entry(hw_mgr_res, &c_ctx->res_list_ife_src, list) {
		...
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			CAM_DBG(CAM_ISP, "add IFE base info for hw %d ctx_idx: %u",
				res->hw_intf->hw_idx, c_ctx->ctx_index);
		}
	}
	...
	CAM_DBG(CAM_ISP, "ctx base num = %d, ctx_idx: %u", c_ctx->num_base, c_ctx->ctx_index);
...
static int cam_ife_mgr_share_sof_qtimer_addr(struct cam_ife_hw_mgr_ctx *ctx)
{
	...
	if (cam_isp_is_ctx_primary_rdi(ctx)) {
		for (i = 0; i < c_ctx->num_acq_vfe_out && !res_rdi_context_set; i++) {
			hw_mgr_res = &c_ctx->res_list_ife_out[i];
	...
	if (!is_found)
		csid_res = list_first_entry(&c_ctx->res_list_ife_csid,
			struct cam_isp_hw_mgr_res, list);

	ife_src_res = list_first_entry(&c_ctx->res_list_ife_src,
		struct cam_isp_hw_mgr_res, list);
	...
	int  grp_index)
{
	...
	uint32_t  res_id = max_ife_out_res, num_out_res;

	if (in_port->per_port_en && (grp_index != CAM_IFE_STREAM_GRP_INDEX_NONE)) {
	...
	for (i = 0; i < num_out_res; i++) {
		...
		if (rc) {
			CAM_ERR(CAM_ISP, "Can not acquire out resource 0x%x, ctx_idx: %u",
				 vfe_out_res_id, c_ctx->ctx_index);
			goto err;
		}
	...
	CAM_DBG(CAM_ISP, "ctx: %d out_res_id = %d, in_res_id: %d",
		c_ctx->ctx_index, vfe_out_res_id, vfe_in_res_id);

	if (i == in_port->num_out_res) {
		CAM_ERR(CAM_ISP,
			"Cannot acquire out resource, i=%d, num_out_res=%d, ctx_idx: %u vfe_out_res_id:%d max_ife_out_res :%d",
			i, in_port->num_out_res, c_ctx->ctx_index, vfe_out_res_id, max_ife_out_res);
		goto err;
	}
	...
	bool                                 update_only)
{
	...
	for (i = 0; i < num_out_res; i++) {
		if (!in_port->ipp_count && per_port_acquire) {
		...
		if ((in_port->major_ver == 3) && (ife_src_res->res_id == CAM_ISP_HW_VFE_IN_CAMIF)) {
			ife_out_res = &c_ctx->res_list_ife_out[c_ctx->vfe_out_map[k]];
			if (ife_out_res) {
		...
		c_ctx->vfe_out_map[k] = c_ctx->num_acq_vfe_out;
		if (per_port_acquire)
			ife_out_res =
				&g_ife_sns_grp_cfg.grp_cfg[grp_index]->res_list_ife_out[k];
		else
			ife_out_res = &c_ctx->res_list_ife_out[c_ctx->num_acq_vfe_out];
...
skip_get_ife_out_res:
		...
		for (j = 0; j < CAM_ISP_HW_SPLIT_MAX; j++) {
			...
			if (!per_port_acquire) {
				if (j == CAM_ISP_HW_SPLIT_LEFT && !update_only) {
					index = vfe_acquire.vfe_out.comp_grp_id;
					comp_grp = &c_ctx->vfe_bus_comp_grp[index];
			...
			CAM_DBG(CAM_ISP, "resource type:0x%x res id:0x%x comp grp id:%d ctx:%u",
				...
				ife_out_res->hw_res[j]->res_id,
				vfe_acquire.vfe_out.comp_grp_id, c_ctx->ctx_index);
		}

		if (!per_port_acquire && !is_ife_out_in_list)
				c_ctx->num_acq_vfe_out++;
	...
	struct cam_isp_in_port_generic_info *in_port)
{
	...
	default:
		CAM_ERR(CAM_ISP,
			...
			sfe_in_res_id, c_ctx->ctx_index);
		goto err;
	}

	CAM_DBG(CAM_ISP,
		"sfe_in_res_id: 0x%x sfe_out_res_id: 0x%x ctx_idx: %u",
		sfe_in_res_id, sfe_out_res_id, c_ctx->ctx_index);

	sfe_acquire.rsrc_type = CAM_ISP_RESOURCE_SFE_OUT;
	sfe_acquire.tasklet = c_ctx->common.tasklet_info;
	c_ctx->sfe_out_map[sfe_out_res_id & 0xFF] = c_ctx->num_acq_sfe_out;
	sfe_out_res = &c_ctx->res_list_sfe_out[c_ctx->num_acq_sfe_out];
	for (i = 0; i < in_port->num_out_res; i++) {
		out_port = &in_port->data[i];
		CAM_DBG(CAM_ISP,
			"i: %d sfe_out_res_id: 0x%x out_port: 0x%x ctx_idx: %u",
			i, sfe_out_res_id, out_port->res_type, c_ctx->ctx_index);
		...
		if (rc) {
			CAM_ERR(CAM_ISP,
				"Can not acquire out resource: 0x%x ctx_idx: %u",
				 out_port->res_type, c_ctx->ctx_index);
			goto err;
		}

		index = sfe_acquire.sfe_out.comp_grp_id;
		comp_grp = &c_ctx->sfe_bus_comp_grp[index];
	...
	if (i == in_port->num_out_res) {
		CAM_ERR(CAM_ISP,
			...
			i, in_port->num_out_res, c_ctx->ctx_index);
		goto err;
	}
	...
	struct cam_isp_in_port_generic_info *in_port)
{
	...
	for (i = 0; i < in_port->num_out_res; i++) {
		...
		CAM_DBG(CAM_ISP, "ctx_idx: %u res_type: 0x%x",
			c_ctx->ctx_index, out_port->res_type);

		c_ctx->sfe_out_map[k] = c_ctx->num_acq_sfe_out;
		sfe_out_res = &c_ctx->res_list_sfe_out[c_ctx->num_acq_sfe_out];
		...
		for (j = 0; j < CAM_ISP_HW_SPLIT_MAX; j++) {
			...
			CAM_DBG(CAM_ISP,
				...
				c_ctx->ctx_index, sfe_out_res->hw_res[j]->res_type,
	...
	list_for_each_entry(sfe_res_iterator,
		...
		default:
			CAM_ERR(CAM_ISP, "Unknown SFE IN resource: %d, ctx_idx: %u",
				sfe_res_iterator->res_id, c_ctx->ctx_index);
			rc = -EINVAL;
			break;
		}
		if (rc)
	...
	int   index)
{
	...
	if (list_empty(ife_src_list_head)) {
		CAM_WARN(CAM_ISP, "IFE src list empty, ctx_idx: %u", c_ctx->ctx_index);
		return 0;
	}
	...
	list_for_each_entry(ife_src_res, ife_src_list_head, list) {
		...
		case CAM_ISP_HW_VFE_IN_LCR:
			rc = cam_ife_hw_mgr_acquire_res_ife_out_pixel(ife_ctx,
				ife_src_res, in_port, index, false);
			break;
		case CAM_ISP_HW_VFE_IN_RDI0:
		case CAM_ISP_HW_VFE_IN_RDI1:
		case CAM_ISP_HW_VFE_IN_RDI2:
		...
		default:
			CAM_ERR(CAM_ISP, "Unknown IFE SRC resource: %d, ctx_idx: %u",
				ife_src_res->res_id, c_ctx->ctx_index);
			break;
		}
		if (rc)
			goto err;
	...
	struct cam_sfe_acquire_args  *sfe_acquire)
{
	...
	if (is_right_hw)
		hw_idx = c_ctx->right_hw_idx;
	else
		hw_idx = c_ctx->left_hw_idx;
	...
	if (rc) {
		CAM_DBG(CAM_ISP,
			...
			hw_idx, sfe_acquire->sfe_in.res_id, c_ctx->ctx_index);
	}
	...
	for (i = sfe_src_res->is_dual_isp; i >= 0; i--) {
		...
		if (rc || !sfe_acquire->sfe_in.rsrc_node) {
			CAM_ERR(CAM_ISP,
				...
				i, sfe_src_res->res_type, sfe_src_res->res_id, c_ctx->ctx_index);
			goto err;
		}
		...
		*acquired_hw_id |=
		...
		CAM_DBG(CAM_ISP,
			...
			sfe_src_res->hw_res[i]->res_id, c_ctx->ctx_index);
	}
...
err:
	CAM_ERR(CAM_ISP, "Acquire SFE failed ctx: %u acquired_res 0x%x sfe_res %u ctx_idx: %u",
		c_ctx->ctx_index, sfe_acquired_res, sfe_res_id, c_ctx->ctx_index);
	...
	struct cam_ife_hw_mgr_ctx                 *ife_ctx)
{
	...
	CAM_DBG(CAM_ISP,
		...
		can_use_lite, csid_acquire->res_id, c_ctx->ctx_index);
	...
	struct cam_isp_in_port_generic_info        *in_port)
{
	...
	if (!ife_hw_mgr->ife_devices[c_ctx->left_hw_idx]) {
		CAM_ERR(CAM_ISP, "ife_devices[%d] is NULL", c_ctx->left_hw_idx);
		return -ENODEV;
	}

	rc = cam_ife_hw_mgr_get_res(&c_ctx->free_res_list, &ife_bus_rd_res);
	if (rc) {
		CAM_ERR(CAM_ISP, "No more free hw mgr resource, ctx_idx: %u", c_ctx->ctx_index);
		return -ENODEV;
	}
	...
	if (c_ctx->left_hw_idx >= CAM_IFE_HW_NUM_MAX) {
	...
	hw_intf = ife_hw_mgr->ife_devices[c_ctx->left_hw_idx]->hw_intf;
	rc = hw_intf->hw_ops.reserve(hw_intf->hw_priv,
		&vfe_acquire, sizeof(struct cam_vfe_acquire_args));
	...
	if (rc || !vfe_acquire.vfe_bus_rd.rsrc_node) {
		...
		CAM_ERR(CAM_ISP, "Failed to acquire VFE:%d BUS RD for LEFT, ctx_idx: %u",
			c_ctx->left_hw_idx, c_ctx->ctx_index);
		goto put_res;
	}
...
acquire_successful:
	...
	if (c_ctx->flags.is_dual) {
		if (!ife_hw_mgr->ife_devices[c_ctx->right_hw_idx]) {
			CAM_ERR(CAM_ISP, "ife_devices[%d] is NULL", c_ctx->right_hw_idx);
			goto put_res;
		}

		hw_intf = ife_hw_mgr->ife_devices[c_ctx->right_hw_idx]->hw_intf;
		rc = hw_intf->hw_ops.reserve(hw_intf->hw_priv,
			&vfe_acquire,
			sizeof(struct cam_vfe_acquire_args));
		...
		if (!rc && vfe_acquire.vfe_bus_rd.rsrc_node) {
			...
			CAM_DBG(CAM_ISP,
				...
				c_ctx->right_hw_idx, c_ctx->ctx_index);
		}
	}
	...
	struct cam_isp_in_port_generic_info *in_port)
{
	...
	if (!cam_ife_hw_mgr_is_sfe_rd_res(in_port->sfe_in_path_type)) {
		CAM_ERR(CAM_ISP, "Invalid sfe rd type: 0x%x ctx_idx: %u",
			in_port->sfe_in_path_type, c_ctx->ctx_index);
		rc = -EINVAL;
		goto put_res;
	}
	...
	if (c_ctx->left_hw_idx >= CAM_SFE_HW_NUM_MAX) {
		if (in_port->ife_rd_count) {
			for (i = 0; i < CAM_SFE_HW_NUM_MAX; i++) {
		...
		if (!ife_hw_mgr->sfe_devices[c_ctx->left_hw_idx]) {
			rc = -ENODEV;
		...
		hw_intf = ife_hw_mgr->sfe_devices[c_ctx->left_hw_idx]->hw_intf;
		rc = hw_intf->hw_ops.reserve(hw_intf->hw_priv,
			&sfe_acquire, sizeof(struct cam_sfe_acquire_args));
	}
	...
	CAM_DBG(CAM_ISP,
		...
		sfe_rd_res->hw_res[0]->res_id, c_ctx->ctx_index);
	...
	uint32_t *acquired_hw_path)
{
	...
	if ((c_ctx->left_hw_idx >= CAM_SFE_HW_NUM_MAX) ||
		(c_ctx->flags.is_dual && (c_ctx->right_hw_idx >= CAM_SFE_HW_NUM_MAX))) {
		CAM_ERR(CAM_ISP, "the hw index:[%d - %d] is wrong",
			c_ctx->left_hw_idx, c_ctx->right_hw_idx);
		goto end;
	}
	...
	if (rc || !vfe_acquire.vfe_in.rsrc_node) {
		CAM_ERR(CAM_ISP, "Unable to acquire LEFT IFE res: %d ctx_idx: %u",
			vfe_acquire.vfe_in.res_id, c_ctx->ctx_index);
		rc = -EINVAL;
		goto put_res;
	}
	...
	*acquired_hw_id |= cam_convert_hw_idx_to_ife_hw_num(hw_intf->hw_idx);
	...
	CAM_DBG(CAM_ISP,
		...
		ife_src_res->hw_res[0]->res_id, c_ctx->ctx_index);
	...
	if (c_ctx->flags.is_dual) {
		...
		*acquired_hw_id |=
		...
		CAM_DBG(CAM_ISP,
			...
			ife_src_res->hw_res[1]->res_id, c_ctx->ctx_index);
	}
...
int cam_ife_hw_mgr_acquire_res_ife_src(
	struct cam_ife_hw_mgr_ctx *ife_ctx,
	struct cam_isp_in_port_generic_info *in_port,
	...
	int   index)
{
	...
	struct list_head                           *csid_res_list_head;
	struct list_head                           *ife_src_list_head;
	bool     per_port_acquire, acquire_lcr_stream_grp = false;
	...
	list_for_each_entry(csid_res, csid_res_list_head, list) {
		...
		rc = cam_ife_hw_mgr_get_res(free_res_list_head,
			&ife_src_res);
		if (rc) {
			CAM_ERR(CAM_ISP, "No more free hw mgr resource, ctx_idx: %u",
				c_ctx->ctx_index);
		...
		cam_ife_hw_mgr_put_res(ife_src_list_head,
			&ife_src_res);
...
skip_get_ife_src_res:
		...
		default:
			CAM_ERR(CAM_ISP, "Wrong IFE CSID Path Resource ID : %d, ctx_idx: %u",
				csid_res->res_id, c_ctx->ctx_index);
			goto err;
		}

		if (csid_res->res_id == CAM_IFE_PIX_PATH_RES_IPP) {
			if (per_port_acquire && !csid_res->is_dual_isp)
				acquire_lcr_stream_grp = true;
		...
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			if (i == CAM_ISP_HW_SPLIT_RIGHT && ife_src_res->is_dual_isp) {
				...
				vfe_acquire.vfe_in.dual_hw_idx = c_ctx->left_hw_idx;
			}
	...
	int  index)
{
	...
	if (compat_count == busy_count)
		CAM_ERR(CAM_ISP, "all compatible CSIDs are busy, ctx_idx: %u", c_ctx->ctx_index);
	...
	struct cam_isp_in_port_generic_info  *in_port)
{
	...
	CAM_DBG(CAM_ISP, "Need CSID PIX %u, Ctx_type: %u, ctx_idx: %u",
		need, c_ctx->ctx_type, c_ctx->ctx_index);
	return need;
}
	...
	int                                  index)
{
	...
	CAM_DBG(CAM_ISP, "CSID Acq: Enter, ctx_idx: %u", c_ctx->ctx_index);
	...
	for (i = csid_res->is_dual_isp; i >= 0 ; i--) {
		CAM_DBG(CAM_ISP, "ctx_idx: %u i %d is_dual %d",
			c_ctx->ctx_index, i, csid_res->is_dual_isp);
		...
		if (per_port_acquire) {
			...
			csid_acquire.decode_format = csid_res->decode_format;
		}
		...
		if (rc) {
			CAM_ERR(CAM_ISP,
				...
				(is_ipp) ? "IPP" : "PPP", c_ctx->ctx_index);
			goto end;
		}
		if (per_port_acquire)
		...
		CAM_DBG(CAM_ISP,
			...
			(i == 0) ? "left" : "right", hw_intf->hw_idx, c_ctx->ctx_index,
			(is_ipp) ? "IPP" : "PPP",
	...
	int                                   index)
{
	...
	if (per_port_acquire) {
		if (cam_ife_hw_mgr_update_vc_dt_stream_grp(csid_res, index, path_res_id, true))
			return -EINVAL;
	}
	...
	if (rc) {
		CAM_ERR(CAM_ISP,
			...
			rc, path_res_id, c_ctx->ctx_index);
		goto put_res;
	}

	if (csid_acquire.node_res == NULL) {
		CAM_ERR(CAM_ISP, "Acquire CSID RDI rsrc failed, ctx_idx: %u", c_ctx->ctx_index);
	...
	CAM_DBG(CAM_ISP,
		...
		CAM_BOOL_TO_YESNO(csid_res->is_secure), c_ctx->ctx_index);
	...
	uint32_t                             *acquired_rdi_res)
{
	...
	if ((res_id != CAM_IFE_PIX_PATH_RES_MAX) && (!(BIT(res_id) & (*acquired_rdi_res)))) {
		rc  = cam_ife_hw_mgr_acquire_csid_rdi_util(ife_ctx,
			in_port, res_id, NULL, CAM_IFE_STREAM_GRP_INDEX_NONE);
		if (rc) {
	...
	CAM_DBG(CAM_ISP, "Ctx: %u rdi_res:%d ctx_type %d rc %d", c_ctx->ctx_index,
	...
	uint32_t                           acquire_info_size)
{
	...
	if (acquire_hw_info->input_info_offset >=
		...
		CAM_ERR(CAM_ISP,
			...
			acquire_hw_info->input_info_size, c_ctx->ctx_index);
		return -EINVAL;
	}
	...
	for (i = 0; i < acquire_hw_info->num_inputs; i++) {
		...
		if (((uint8_t *)in_port +
			...
			acquire_info_size)) {
			CAM_ERR(CAM_ISP, "Invalid size, ctx_idx: %u", c_ctx->ctx_index);
			return -EINVAL;
		}
		...
		if ((in_port->num_out_res > max_ife_out_res) ||
			...
			CAM_ERR(CAM_ISP, "Invalid num output res %u ctx_idx: %u",
				in_port->num_out_res, c_ctx->ctx_index);
			return -EINVAL;
		}
		...
		if (in_port->res_type == CAM_ISP_IFE_IN_RES_RD) {
			ife_ctx->concr_ctx->flags.is_fe_enabled = true;
			break;
		}
	...
	CAM_DBG(CAM_ISP, "is_fe_enabled %d, ctx_idx: %u",
		ife_ctx->concr_ctx->flags.is_fe_enabled, c_ctx->ctx_index);
	...
	struct cam_ife_mgr_bw_data        *bw_data)
{
	...
	CAM_DBG(CAM_ISP,
		"fe_enabled %d offline %d sfe_fs %d sfe_shdr: %d num_sfe_fetches: %u ctx_idx: %u",
		c_ctx->flags.is_fe_enabled,
	...
	struct cam_ife_mgr_bw_data        *bw_data)
{
	...
	case 3:
		...
		break;
	default:
		CAM_ERR(CAM_ISP, "Invalid ver of user common info:  minor %u major %u",
			minor_ver, major_ver);
		return -EINVAL;
	}
	...
	uint32_t                    *max_height)
{
	...
	for (i = 0; i < in_port->num_out_res; i++) {
		...
		if (cam_ife_hw_mgr_is_rdi_res(out_port->res_type)) {
			...
			if (out_port->height >= *max_height) {
				c_ctx->pri_rdi_out_res = out_port->res_type;
	...
	CAM_DBG(CAM_ISP, "ife_ctx_idx: %u rdi: %d ipp: %d ppp: %d ife_rd: %d lcr: %d",
		c_ctx->ctx_index, in_port->rdi_count, in_port->ipp_count,
	...
	uint32_t                                   *acquired_hw_path)
{
	...
	if (rc) {
		CAM_ERR(CAM_ISP, "No free resource, ctx_idx: %u", c_ctx->ctx_index);
		goto end;
	}
	...
	*acquired_hw_id |= cam_convert_hw_idx_to_ife_hw_num(
	...
	CAM_DBG(CAM_ISP, "Acquired VFE:%d CAMIF for LEFT, ctx_idx: %u",
		ife_src_res->hw_res[0]->hw_intf->hw_idx, c_ctx->ctx_index);
	...
	if (c_ctx->flags.is_dual) {
		...
		*acquired_hw_id |= cam_convert_hw_idx_to_ife_hw_num(
		...
		CAM_DBG(CAM_ISP, "Acquired VFE:%d CAMIF for RIGHT, ctx_idx: %u",
			ife_src_res->hw_res[1]->hw_intf->hw_idx, c_ctx->ctx_index);
	}
	...
	struct cam_isp_in_port_generic_info        *in_port)
{
	...
	sfe_acquire.tasklet = c_ctx->common.tasklet_info;
	if (ife_ctx->is_offline) {
		sfe_acquire.priv = ife_hw_mgr;
		sfe_acquire.event_cb = cam_ife_hw_mgr_event_handler_offline;
	} else {
		sfe_acquire.priv = ife_ctx;
		sfe_acquire.event_cb = cam_ife_hw_mgr_event_handler;
	}
	...
	if (rc) {
		CAM_ERR(CAM_ISP,
			"Failed to acquire SFE PIX for offline, ctx_idx: %u", c_ctx->ctx_index);
		goto put_res;
	}

	sfe_src_res->hw_res[i] = sfe_acquire.sfe_in.rsrc_node;
	CAM_DBG(CAM_ISP, "Acquired SFE: %u PIX LEFT for offline, ctx_idx: %u",
		sfe_src_res->hw_res[i]->hw_intf->hw_idx, c_ctx->ctx_index);
	...
	if (c_ctx->flags.is_dual) {
		CAM_WARN(CAM_ISP,
			"DUAL not supported for offline use-case, ctx_idx: %u", c_ctx->ctx_index);
	...
	struct cam_isp_in_port_generic_info        *in_port)
{
	...
	if (ife_ctx->is_offline) {
		csid_acquire.cb_priv = c_ctx->hw_mgr;
		csid_acquire.event_cb = cam_ife_hw_mgr_event_handler_offline;
	} else {
		csid_acquire.cb_priv = ife_ctx;
		csid_acquire.event_cb = cam_ife_hw_mgr_event_handler;
	}
	...
	csid_res->hw_res[0] = csid_acquire.node_res;
	csid_res->hw_res[1] = NULL;
	c_ctx->left_hw_idx  = csid_acquire.node_res->hw_intf->hw_idx;
	...
	uint32_t                            *acquired_hw_path)
{
	...
	if (rc) {
		CAM_ERR(CAM_ISP,
			"Acquire IFE CSID RDI0 resource Failed, ctx_idx: %u", c_ctx->ctx_index);
			goto err;
	}
	...
	if (rc) {
		CAM_ERR(CAM_ISP,
			"Acquire SFE PIX SRC resource Failed, ctx_idx: %u", c_ctx->ctx_index);
		goto err;
	}
	...
	if (rc) {
		CAM_ERR(CAM_ISP,
			"Acquire SFE OUT resource Failed, ctx_idx: %u", c_ctx->ctx_index);
			goto err;
	}
	...
	uint32_t                            *acquired_hw_path)
{
	...
	if (in_port->rdi_count || in_port->ppp_count) {
		CAM_ERR(CAM_ISP,
			...
			in_port->rdi_count, in_port->ppp_count, c_ctx->ctx_index);
		return -EINVAL;
	}
	...
	uint32_t *acquired_rdi_res)
{
	...
	if (rc) {
		CAM_ERR(CAM_ISP, "Can not acquire root resource, ctx_idx: %u", c_ctx->ctx_index);
		goto err;
	}
	...
	if (!in_port->ipp_count && !in_port->rdi_count &&
		...
		CAM_ERR(CAM_ISP,
			"No PIX or RDI or PPP or LCR resource, ctx_idx: %u", c_ctx->ctx_index);
		return -EINVAL;
	}
	...
	if (in_port->per_port_en && c_ctx->ctx_type == CAM_IFE_CTX_TYPE_NONE &&
		...
		CAM_DBG(CAM_ISP,
			...
			in_port->per_port_en, in_port->sensor_id, c_ctx->ctx_type);
	}
	...
	if (in_port->rdi_count) {
		...
		rc = cam_ife_hw_mgr_acquire_res_ife_csid_rdi(ife_ctx, in_port,
			acquired_rdi_res,
			CAM_IFE_STREAM_GRP_INDEX_NONE, false);
		if (rc) {
			CAM_ERR(CAM_ISP,
	...
	CAM_DBG(CAM_ISP, "Acquired CSID resource, is_dual:%s left_hw_idx:%d right_hw_idx:%d",
		CAM_BOOL_TO_YESNO(c_ctx->flags.is_dual),
		c_ctx->left_hw_idx, c_ctx->right_hw_idx);
	...
	if (in_port->ife_rd_count) {
		...
		if (rc) {
			CAM_ERR(CAM_ISP, "Acquire %s BUS RD resource Failed, ctx_idx: %u",
				(c_ctx->ctx_type ==
				CAM_IFE_CTX_TYPE_SFE) ? "SFE" : "IFE", c_ctx->ctx_index);
			goto err;
		}
	}
	...
	enum cam_cdm_cb_status status, void *cookie)
{
	...
	c_ctx = ctx->concr_ctx;
	hw_update_data = c_ctx->cdm_userdata.hw_update_data;

	if (status == CAM_CDM_CB_STATUS_BL_SUCCESS) {
		req_id = *(uint64_t *)cookie;
		complete_all(&c_ctx->config_done_complete);
		atomic_set(&c_ctx->cdm_done, 1);
		c_ctx->last_cdm_done_req = req_id;
		if (g_ife_hw_mgr.debug_cfg.per_req_reg_dump) {
			if (c_ctx->cdm_userdata.request_id == req_id) {
				...
				CAM_INFO(CAM_ISP,
				    ...
				    req_id, c_ctx->cdm_userdata.request_id, c_ctx->ctx_index);
			}
		}
		CAM_DBG(CAM_ISP,
			"CDM hdl=0x%x, udata=%pK, status=%d, cookie=%u ctx_index=%u cdm_req=%llu",
			 handle, userdata, status, req_id, c_ctx->ctx_index,
			 c_ctx->cdm_userdata.request_id);
		...
		CAM_WARN(CAM_ISP,
			"Called by CDM hdl=0x%x, udata=%pK, status=%d, cdm_req=%llu ctx_idx: %u",
			 handle, userdata, status,
			 c_ctx->cdm_userdata.request_id, c_ctx->ctx_index);
	}
	ktime_get_clocktai_ts64(&c_ctx->cdm_done_ts);
}
	...
	struct cam_isp_in_port_generic_info *in_port)
{
	...
	*input_size += in_port_length;
	...
	for (i = 0; i < in->num_out_res; i++) {
		...
		in_port->data[i].acquired_res_type = in->data_flex[i].res_type;
	...
	struct cam_isp_in_port_generic_info *in_port)
{
	...
	*input_size += in_port_length;
	...
	if (in_port->per_port_en && !in_port->is_offline) {
	...
	struct cam_ife_hw_mgr_ctx *ctx)
{
	...
	c_ctx->scratch_buf_info.ife_scratch_config->skip_scratch_cfg_streamon = false;
	c_ctx->scratch_buf_info.sfe_scratch_config->skip_scratch_cfg_streamon = false;
	c_ctx->scratch_buf_info.ife_scratch_config->streamon_buf_mask = 0;
	c_ctx->scratch_buf_info.sfe_scratch_config->streamon_buf_mask = 0;
}
	...
	struct cam_isp_in_port_generic_info *in_port)
{
	...
	c_ctx->acq_hw_ctxt_src_dst_map[src_hw_ctxt_id] = in_port->ipp_dst_hw_ctxt_mask;
}

int cam_ife_mgr_update_offline_ife_out(
	struct cam_ife_hw_mgr_ctx *ife_ctx)
{
	struct cam_isp_hw_mgr_res       *ife_src_res;
	int i;
...
static int cam_ife_mgr_acquire_hw(void *hw_mgr_priv, void *acquire_hw_args)
{
	...
	if (rc || !c_ctx) {
		CAM_ERR(CAM_ISP, "Get ife hw context failed rc %d", rc);
		goto err;
	}

	cam_cpas_get_cpas_hw_version(&c_ctx->hw_version);
	c_ctx->ctx_config = 0;
	c_ctx->cdm_handle = 0;
	...
	c_ctx->common.cb_priv = acquire_args->context_data;
	c_ctx->common.mini_dump_cb = acquire_args->mini_dump_cb;
	c_ctx->flags.internal_cdm = false;
	c_ctx->left_hw_idx = CAM_IFE_CSID_HW_NUM_MAX;
	c_ctx->right_hw_idx = CAM_IFE_CSID_HW_NUM_MAX;
	c_ctx->buf_done_controller = NULL;
	c_ctx->common.event_cb = acquire_args->event_cb;
	c_ctx->hw_mgr = ife_hw_mgr;
	c_ctx->cdm_ops =  cam_cdm_publish_ops();
	c_ctx->common.sec_pf_evt_cb = acquire_args->sec_pf_evt_cb;
	c_ctx->try_recovery_cnt = 0;
	c_ctx->recovery_req_id = 0;
	c_ctx->drv_path_idle_en = 0;
	c_ctx->res_list_ife_out = NULL;
	c_ctx->res_list_sfe_out = NULL;
	c_ctx->left_hw_idx = CAM_IFE_CSID_HW_NUM_MAX;
	c_ctx->right_hw_idx = CAM_IFE_CSID_HW_NUM_MAX;
	c_ctx->pri_rdi_out_res = g_ife_hw_mgr.isp_caps.max_vfe_out_res_type;
	...
	c_ctx->is_offline = c_ctx->flags.is_offline;
	...
	if (!ife_mgr_ctx->in_ports) {
		CAM_ERR(CAM_ISP, "No memory available, ctx_idx: %u", c_ctx->ctx_index);
		rc = -ENOMEM;
		goto free_ctx;
	}

	c_ctx->vfe_bus_comp_grp = kcalloc(CAM_IFE_BUS_COMP_NUM_MAX,
		sizeof(struct cam_isp_context_comp_record), GFP_KERNEL);
	if (!c_ctx->vfe_bus_comp_grp) {
		CAM_ERR(CAM_CTXT, "No memory for vfe_bus_comp_grp");
		rc = -ENOMEM;
		goto free_mem;
	}

	c_ctx->sfe_bus_comp_grp = kcalloc(CAM_SFE_BUS_COMP_NUM_MAX,
		sizeof(struct cam_isp_context_comp_record), GFP_KERNEL);
	if (!c_ctx->sfe_bus_comp_grp) {
	...
	for (i = 0; i < acquire_hw_info->num_inputs; i++) {
		ife_mgr_ctx->in_ports[i].is_offline = c_ctx->is_offline;
		rc = cam_ife_mgr_acquire_get_unified_structure(acquire_hw_info,
		...
		if (c_ctx->flags.is_fe_enabled) {
			...
			for (j = 1; j < ife_mgr_ctx->in_ports[i].num_valid_vc_dt; j++) {
				if (ife_mgr_ctx->in_ports[i].format[j] !=
					input_format_checker) {
					CAM_ERR(CAM_ISP,
						"Different input formats for FE use-cases not supported - formats vc0: %u vc%d: %u ctx_idx: %u",
						input_format_checker, j,
						ife_mgr_ctx->in_ports[i].format[j],
						c_ctx->ctx_index);
					rc = -EINVAL;
					goto free_mem;
				}
		...
		if (ife_mgr_ctx->in_ports[i].usage_type &&
			ife_mgr_ctx->in_ports[i].secure_mode) {
			CAM_ERR(CAM_ISP,
		...
		cam_ife_hw_mgr_preprocess_port(ife_mgr_ctx,
				&ife_mgr_ctx->in_ports[i], &max_height);
		...
		total_sfe_ports += ife_mgr_ctx->in_ports[i].sfe_port_count;

		if ((ife_mgr_ctx->in_ports[i].major_ver == 3) &&
			ife_mgr_ctx->in_ports[i].ipp_count)
			c_ctx->is_hw_ctx_acq = true;
	}

	total_ports = total_pix_port + total_rdi_port + total_pd_port;

	in_port = &ife_mgr_ctx->in_ports[0];
	for (i = 0; i < in_port->num_valid_vc_dt; i++) {
	...
	c_ctx->sensor_id = in_port[0].sensor_id;
	c_ctx->flags.per_port_en = in_port->per_port_en;
	ife_mgr_ctx->virtual_rdi_mapping_cb =
		cam_ife_hw_mgr_get_virtual_mapping_out_port;

	c_ctx->res_list_ife_out = kcalloc(total_ports,
		sizeof(struct cam_isp_hw_mgr_res), GFP_KERNEL);
	if (!c_ctx->res_list_ife_out) {
		rc = -ENOMEM;
	...
	if (!total_pix_port && !total_pd_port) {
		c_ctx->flags.is_rdi_only_context = true;
		CAM_DBG(CAM_ISP, "RDI only context, ctx_idx: %u", c_ctx->ctx_index);
	}

	/* Check whether context has only RDI and PD resource */
	if (!total_pix_port &&
		(total_pd_port && total_rdi_port)) {
		c_ctx->flags.rdi_pd_context = true;
		CAM_DBG(CAM_ISP, "RDI and PD context with [%u pd] [%u rdi], ctx_idx: %u",
			total_pd_port, total_rdi_port, c_ctx->ctx_index);
	}

	/* Check if all output ports are of lite  */
	if (total_lite_port == total_pix_port + total_rdi_port)
		c_ctx->flags.is_lite_context = true;

	if (total_sfe_ports) {
		c_ctx->res_list_sfe_out = kcalloc(total_sfe_ports,
			sizeof(struct cam_isp_hw_mgr_res), GFP_KERNEL);
		memset(c_ctx->sfe_out_map, 0xff, sizeof(uint8_t) * max_sfe_out_res);
		if (!c_ctx->res_list_sfe_out) {
			rc = -ENOMEM;
	...
	for (i = 0; i < acquire_hw_info->num_inputs; i++) {
		CAM_DBG(CAM_ISP, "ctx_idx: %u in_res_type %x",
			c_ctx->ctx_index, ife_mgr_ctx->in_ports[i].res_type);
		if (!c_ctx->ctx_type) {
			if (ife_mgr_ctx->in_ports[i].cust_node) {
				c_ctx->ctx_type = CAM_IFE_CTX_TYPE_CUSTOM;
		...
		CAM_DBG(CAM_ISP,
			"in_res_type: 0x%x ctx_idx: %u sfe_in_path_type: 0x%x sfe_ife_enable: 0x%x per_port_en :%d",
			in_port[i].res_type, c_ctx->ctx_index,  in_port[i].sfe_in_path_type,
			in_port[i].sfe_ife_enable, c_ctx->flags.per_port_en);

		/* For multi-context acquire cases*/
		if ((ife_mgr_ctx->in_ports[i].major_ver == 3) &&
			ife_mgr_ctx->in_ports[i].ipp_count)
			cam_ife_mgr_populate_hw_ctxt_map(ife_mgr_ctx, &ife_mgr_ctx->in_ports[i]);
	...
	if (rc) {
		CAM_ERR(CAM_ISP, "Process base info failed, ctx_idx: %u", c_ctx->ctx_index);
		goto free_res;
	}
	...
	if (!ife_hw_mgr->csid_camif_irq_support || (ife_hw_mgr->csid_camif_irq_support &&
		c_ctx->ctx_type == CAM_IFE_CTX_TYPE_SFE)) {
		rc = cam_ife_mgr_share_sof_qtimer_addr(ife_mgr_ctx);
		if (rc)
			goto free_res;
	}

	rc = cam_ife_mgr_allocate_cdm_cmd(
		(c_ctx->ctx_type == CAM_IFE_CTX_TYPE_SFE ? true : false), &c_ctx->cdm_cmd);
	if (rc)
		goto free_res;
	...
	if (c_ctx->flags.is_dual)
		cdm_acquire.cell_index = c_ctx->left_hw_idx;
	else
		cdm_acquire.cell_index = c_ctx->base[0].idx;
	...
	if (rc) {
		CAM_ERR(CAM_ISP, "Failed to acquire the CDM HW, ctx_idx: %u", c_ctx->ctx_index);
		goto free_cdm_cmd;
	}

	CAM_DBG(CAM_ISP,
		"Successfully acquired ctx_idx: %u CDM Id: %d, CDM HW hdl=%x, is_dual=%d",
		c_ctx->ctx_index, cdm_acquire.id, cdm_acquire.handle, c_ctx->flags.is_dual);
	c_ctx->cdm_handle = cdm_acquire.handle;
	c_ctx->cdm_id = cdm_acquire.id;
	c_ctx->cdm_hw_idx = cdm_acquire.hw_idx;
	if (cdm_acquire.id == CAM_CDM_IFE)
		c_ctx->flags.internal_cdm = true;
	atomic_set(&c_ctx->cdm_done, 1);
	atomic_set(&c_ctx->ctx_state, CAM_IFE_HW_STATE_STOPPED);
	...
	if ((c_ctx->flags.is_sfe_shdr) ||
		...
		c_ctx->scratch_buf_info.ife_scratch_config =
			kzalloc(sizeof(struct cam_ife_scratch_buf_cfg), GFP_KERNEL);
		if (!c_ctx->scratch_buf_info.ife_scratch_config) {
	...
	acquire_args->hw_mgr_ctx_id = c_ctx->ctx_index;
...
free_mem:
	...
	kfree(c_ctx->vfe_bus_comp_grp);
	kfree(c_ctx->sfe_bus_comp_grp);
	c_ctx->vfe_bus_comp_grp = NULL;
	c_ctx->sfe_bus_comp_grp = NULL;

	kfree(c_ctx->res_list_sfe_out);
	c_ctx->res_list_sfe_out = NULL;
	kfree(c_ctx->res_list_ife_out);
	c_ctx->res_list_ife_out = NULL;
free_ctx:
...
static int cam_ife_mgr_acquire_dev(void *hw_mgr_priv, void *acquire_hw_args)
{
	...
	for (i = 0; i < acquire_args->num_acq; i++) {
		...
		CAM_DBG(CAM_ISP, "ctx_idx: %u acquire no = %d total = %d", c_ctx->ctx_index, i,
			acquire_args->num_acq);
		CAM_DBG(CAM_ISP,
			"ctx_idx: %u start copy from user handle %lld with len = %d",
			c_ctx->ctx_index, isp_resource[i].res_hdl,
	...
	CAM_DBG(CAM_ISP, "Successfully acquired CDM ID:%d, CDM HW hdl=%x ctx_idx: %u",
		cdm_acquire.id, cdm_acquire.handle, c_ctx->ctx_index);
	...
	struct cam_isp_prepare_hw_update_data *hw_update_data)
{
	...
	CAM_INFO(CAM_ISP, "ctx: %u req_id:%llu config_valid[BW VFE_CLK SFE_CLK]:[%d %d %d]",
		c_ctx->ctx_index, request_id, hw_update_data->bw_clk_config.bw_config_valid,
	...
	for (i = 0; i < bw_config->num_paths; i++) {
		CAM_INFO(CAM_PERF,
			...
			c_ctx->ctx_index, bw_config->usage_type,
...
ife_clk:
	...
	CAM_INFO(CAM_PERF, "IFE ctx_idx: %u clk update usage=%u left_clk= %lu right_clk=%lu",
		c_ctx->ctx_index, ife_clock_config->usage_type, ife_clock_config->left_pix_hz,
...
sfe_clk:
	...
	CAM_INFO(CAM_PERF, "SFE ctx_idx: %u clk update usage: %u left_clk: %lu right_clk: %lu",
		c_ctx->ctx_index, sfe_clock_config->usage_type, sfe_clock_config->left_pix_hz,
	...
	list_for_each_entry(hw_mgr_res, &c_ctx->res_list_ife_src, list) {
		for (split_idx = 0; split_idx < CAM_ISP_HW_SPLIT_MAX;
			...
			if (hw_intf && hw_intf->hw_ops.process_cmd) {
				...
				CAM_WARN(CAM_ISP, "NULL hw_intf!, ctx_idx: %u", c_ctx->ctx_index);
			}
		}
	}
			...
			if ((hw_mgr_res->res_id == CAM_ISP_HW_VFE_IN_CAMIF) ||
				...
				if (hw_mgr_res->hw_res[i]) {
					CAM_ERR(CAM_ISP, "Invalid ctx_idx: %u res_id %u",
						c_ctx->ctx_index, hw_mgr_res->res_id);
					rc = -EINVAL;
					return rc;
				}
			}
	...
	void *config_hw_args)
{
	...
	if (!c_ctx->flags.ctx_in_use || !c_ctx->cdm_cmd) {
		CAM_ERR(CAM_ISP,
			...
			c_ctx->ctx_index, c_ctx->flags.ctx_in_use, c_ctx->cdm_cmd);
		return -EPERM;
	}
	...
	if (cfg->reapply_type && cfg->cdm_reset_before_apply) {
		if (c_ctx->last_cdm_done_req < cfg->request_id) {
			...
			CAM_ERR_RATE_LIMIT(CAM_ISP,
				...
				c_ctx->ctx_index, cfg->request_id, c_ctx->last_cdm_done_req,
				is_cdm_hung);
	...
	if (hw_update_data->bw_clk_config.bw_config_valid) {
		if (c_ctx->bw_config_version == CAM_ISP_BW_CONFIG_V1) {
			...
			CAM_ERR(CAM_PERF, "Invalid bw config version: %d, ctx_idx=%u",
				c_ctx->bw_config_version, c_ctx->ctx_index);
		}
	}
...
skip_bw_clk_update:
	rc = cam_ife_mgr_apply_fcg_update(hw_mgr_ctx, hw_update_data, cfg);
	if (rc) {
		CAM_ERR(CAM_ISP, "Failed in updating FCG values %u", c_ctx->ctx_index);
		return rc;
	}
	...
	if (cfg->num_hw_update_entries > 0) {
		...
		for (i = 0 ; i < cfg->num_hw_update_entries; i++) {
			...
			if (hw_update_data->fcg_info.ife_fcg_online &&
				...
				CAM_DBG(CAM_ISP,
					...
					i, c_ctx->ctx_index, cfg->request_id,
					hw_update_data->fcg_info.use_current_cfg);
			...
			if (g_ife_hw_mgr.debug_cfg.enable_cdm_cmd_check) {
				...
				if (rc) {
					CAM_ERR(CAM_ISP,
						...
						cdm_cmd->cmd_flex[i - skip].bl_addr.mem_handle,
						c_ctx->ctx_index, cfg->request_id);
					continue;
				}
				...
				if ((i == 0) && (cmd_type != CAM_CDM_CMD_CHANGE_BASE)) {
					CAM_ERR(CAM_ISP,
						...
						cmd_type, c_ctx->ctx_index, cfg->request_id);
		...
		CAM_DBG(CAM_ISP, "Submit to CDM, ctx_idx=%u", c_ctx->ctx_index);
		...
		CAM_ERR(CAM_ISP, "No commands to config, ctx_index %u", c_ctx->ctx_index);
	}

	CAM_DBG(CAM_ISP, "Exit: Config Done: %llu, ctx_index %u",
				cfg->request_id, c_ctx->ctx_index);
	return rc;
}
...
static int cam_ife_mgr_stop_hw_in_overflow(void *stop_hw_args)
{
	...
	if (!c_ctx->num_base) {
		CAM_ERR(CAM_ISP, "Number of bases are zero, ctx_index %u", c_ctx->ctx_index);
		return -EINVAL;
	}
	...
	if (i == c_ctx->num_base)
		master_base_idx = c_ctx->base[0].idx;

	/* stop the master CSID path first */
	cam_ife_mgr_csid_stop_hw(hw_mgr_ctx, &c_ctx->res_list_ife_csid,
	...
	for (i = 0; i < c_ctx->num_base; i++) {
		...
		cam_ife_mgr_csid_stop_hw(hw_mgr_ctx, &c_ctx->res_list_ife_csid,
			c_ctx->base[i].idx, CAM_CSID_HALT_IMMEDIATELY);
	}
	...
	/* IFE out resources */
	for (i = 0; i < c_ctx->num_acq_vfe_out; i++)
		cam_ife_hw_mgr_stop_hw_res(&c_ctx->res_list_ife_out[i]);

	/* Stop tasklet for context */
	cam_tasklet_stop(c_ctx->common.tasklet_info);
	...
	list_for_each_entry(hw_mgr_res, &c_ctx->res_list_ife_src, list) {
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			if (hw_intf && hw_intf->hw_ops.process_cmd) {
				...
				CAM_WARN(CAM_ISP, "NULL hw_intf!, ctx id:%u", c_ctx->ctx_index);
		}
	}
...
static int cam_ife_mgr_stop_hw(void *hw_mgr_priv, void *stop_hw_args)
{
	...
	if (!c_ctx->num_base) {
		CAM_ERR(CAM_ISP, "number of bases are zero, ctx id:%u", c_ctx->ctx_index);
		return -EINVAL;
	}

	/* Cancel all scheduled recoveries without affecting future recoveries */
	atomic_inc(&c_ctx->recovery_id);
	...
	if ((stop_isp->hw_stop_cmd == CAM_ISP_HW_STOP_AT_FRAME_BOUNDARY) ||
		c_ctx->flags.dsp_enabled)
		csid_halt_type = CAM_CSID_HALT_AT_FRAME_BOUNDARY;
	else
		csid_halt_type = CAM_CSID_HALT_IMMEDIATELY;

	if (c_ctx->flags.per_port_en && !c_ctx->flags.is_dual && !c_ctx->flags.is_offline) {
		rc = cam_ife_hw_mgr_res_stream_on_off_grp_cfg(hw_mgr_ctx,
	...
	CAM_DBG(CAM_ISP, "Halting CSIDs, ctx id:%u", c_ctx->ctx_index);
	...
	CAM_DBG(CAM_ISP, "Stopping master CSID idx %d, ctx id:%u",
			master_base_idx, c_ctx->ctx_index);
	...
	/* stop rest of the CSID paths  */
	for (i = 0; i < c_ctx->num_base; i++) {
		if (c_ctx->base[i].idx == master_base_idx)
			continue;
		CAM_DBG(CAM_ISP, "Stopping CSID idx %d i %d master %d ctx id:%u",
			c_ctx->base[i].idx, i, master_base_idx, c_ctx->ctx_index);

		cam_ife_mgr_csid_stop_hw(hw_mgr_ctx, &c_ctx->res_list_ife_csid,
			c_ctx->base[i].idx, csid_halt_type);
	}
	...
	if (c_ctx->ctx_type == CAM_IFE_CTX_TYPE_SFE) {
		CAM_DBG(CAM_ISP, "Going to stop SFE Out, ctx id:%u", c_ctx->ctx_index);

		/* SFE out resources */
		for (i = 0; i < c_ctx->num_acq_sfe_out; i++)
			cam_ife_hw_mgr_stop_hw_res(&c_ctx->res_list_sfe_out[i]);

		CAM_DBG(CAM_ISP, "Going to stop SFE SRC resources, ctx id:%u", c_ctx->ctx_index);
	...
	CAM_DBG(CAM_ISP, "Going to stop IFE out resources, ctx id:%u", c_ctx->ctx_index);

	/* IFE out resources */
	for (i = 0; i < c_ctx->num_acq_vfe_out; i++)
		cam_ife_hw_mgr_stop_hw_res(&c_ctx->res_list_ife_out[i]);

	CAM_DBG(CAM_ISP, "Going to stop IFE Mux, ctx id:%u", c_ctx->ctx_index);
...
reset_scratch_buffers:
	...
	if (!stop_isp->is_internal_stop) {
		c_ctx->current_mup = 0;
		if (c_ctx->scratch_buf_info.sfe_scratch_config)
			memset(c_ctx->scratch_buf_info.sfe_scratch_config, 0,
				sizeof(struct cam_sfe_scratch_buf_cfg));

		if (c_ctx->scratch_buf_info.ife_scratch_config)
			memset(c_ctx->scratch_buf_info.ife_scratch_config, 0,
	...
	if (rem_jiffies == 0)
		CAM_WARN(CAM_ISP,
			...
			c_ctx->applied_req_id, c_ctx->ctx_index);
	...
	if (stop_isp->is_internal_stop) {
		...
		if (rc) {
			CAM_WARN(CAM_ISP, "CDM: %u reset failed rc: %d in ctx: %u",
				c_ctx->cdm_id, rc, c_ctx->ctx_index);
			rc = 0;
		}
	}
	...
	if (cam_cdm_stream_off(c_ctx->cdm_handle))
		CAM_ERR(CAM_ISP, "CDM stream off failed %d, ctx_id: %u",
			c_ctx->cdm_handle, c_ctx->ctx_index);

	if (!skip_hw_deinit)
		cam_ife_hw_mgr_deinit_hw(hw_mgr_ctx);

	CAM_DBG(CAM_ISP,
		"Stop success for ctx id:%d rc :%d", c_ctx->ctx_index, rc);
...
static int cam_ife_mgr_restart_hw(void *start_hw_args)
{
	...
	CAM_DBG(CAM_ISP, "START IFE OUT ... in ctx id:%u", c_ctx->ctx_index);

	cam_tasklet_start(c_ctx->common.tasklet_info);

	/* start the IFE out devices */
	for (i = 0; i < c_ctx->num_acq_vfe_out; i++) {
		rc = cam_ife_hw_mgr_start_hw_res(
			&c_ctx->res_list_ife_out[i], hw_mgr_ctx);
		if (rc) {
	...
	CAM_DBG(CAM_ISP, "START IFE SRC ... in ctx id:%u", c_ctx->ctx_index);
	...
	CAM_DBG(CAM_ISP, "START CSID HW ... in ctx id:%u", c_ctx->ctx_index);

	/* Start the IFE CSID HW devices */
	rc = cam_ife_mgr_csid_start_hw(hw_mgr_ctx, CAM_IFE_PIX_PATH_RES_MAX, false, false);
	if (rc) {
		CAM_ERR(CAM_ISP, "Error in starting CSID HW in ctx id:%u", c_ctx->ctx_index);
		goto err;
	}

	/* Start IFE root node: do nothing */
	CAM_DBG(CAM_ISP, "Exit...(success), ctx_idx: %u", c_ctx->ctx_index);
	return 0;

err:
	cam_ife_mgr_stop_hw_in_overflow(start_hw_args);
	CAM_DBG(CAM_ISP, "Exit...(rc=%d), ctx_idx: %u", rc, c_ctx->ctx_index);
	return rc;
}
	...
	uint32_t *core_idx)
{
	...
	for (i = 0; i < c_ctx->num_base; i++) {
		...
		if (c_ctx->base[i].split_id == split_id) {
			CAM_DBG(CAM_ISP, "Found core: %u for split_id: %d hw_type: %d ctx_idx: %u",
				c_ctx->base[i].idx, split_id, hw_type, c_ctx->ctx_index);
	...
	CAM_ERR(CAM_ISP, "Failed to find core idx for hw_type: %d split_id %d ctx_idx: %u",
		hw_type, split_id, c_ctx->ctx_index);
	return -EINVAL;
...
static int cam_ife_mgr_start_hw(void *hw_mgr_priv, void *start_hw_args)
{
	...
	if ((!c_ctx->flags.init_done) && start_isp->start_only) {
		CAM_ERR(CAM_ISP, "Invalid args init_done %d start_only %d, ctx_idx: %u",
			c_ctx->flags.init_done, start_isp->start_only, c_ctx->ctx_index);
		return -EINVAL;
	}

	CAM_DBG(CAM_ISP, "Enter... ctx id:%u c_ctx->flags.need_csid_top_cfg:%d",
		c_ctx->ctx_index, c_ctx->flags.need_csid_top_cfg);
	...
	if (c_ctx->flags.need_csid_top_cfg) {
		list_for_each_entry(hw_mgr_res, &c_ctx->res_list_ife_csid,
				list) {
			for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
				if (!hw_mgr_res->hw_res[i])
					continue;
				...
				if (c_ctx->ctx_type == CAM_IFE_CTX_TYPE_SFE) {
					...
					rc = cam_ife_mgr_find_core_idx(
						i, hw_mgr_ctx, CAM_ISP_HW_TYPE_SFE,
				...
				CAM_DBG(CAM_ISP,
					...
					csid_top_args.input_core_type,
					csid_top_args.is_sfe_offline, c_ctx->ctx_index);
			}
		}
	}
	...
	if (rc) {
		CAM_ERR(CAM_ISP, "Init failed, ctx_idx: %u", c_ctx->ctx_index);
		goto tasklet_stop;
	}
	...
	rc = cam_cdm_stream_on(c_ctx->cdm_handle);
	if (rc) {
...
start_only:
	...
	if (cam_isp_is_ctx_primary_rdi(hw_mgr_ctx) && (c_ctx->pri_rdi_out_res <
		g_ife_hw_mgr.isp_caps.max_vfe_out_res_type)) {
		hw_mgr_res =
			&c_ctx->res_list_ife_out[c_ctx->vfe_out_map[c_ctx->pri_rdi_out_res & 0xff]];
		hw_mgr_res->hw_res[0]->is_rdi_primary_res =  true;
		primary_rdi_src_res =
			cam_convert_rdi_out_res_id_to_src(c_ctx->pri_rdi_out_res);
		primary_rdi_csid_res =
			cam_ife_hw_mgr_get_ife_csid_rdi_res_type(c_ctx->pri_rdi_out_res);
	}
	...
	/* Start IFE root node: do nothing */
	CAM_DBG(CAM_ISP, "Start success for ctx id:%d", c_ctx->ctx_index);
...
cdm_streamoff:
	cam_cdm_stream_off(c_ctx->cdm_handle);
safe_disable:
#ifdef CONFIG_SPECTRA_SECURE
					...
					void *release_hw_args)
{
	...
	if (c_ctx->flags.init_done && !skip_deinit_hw)
		cam_ife_hw_mgr_deinit_hw(hw_mgr_ctx);

	/* we should called the stop hw before this already */
	if (!per_port_feature_enable)
		cam_ife_hw_mgr_release_hw_for_ctx(hw_mgr_ctx, CAM_IFE_STREAM_GRP_INDEX_NONE);
	...
	c_ctx->num_acq_vfe_out = 0;
	c_ctx->num_acq_sfe_out = 0;
	c_ctx->mapping_table.rdi_path_count = 0;
	for (i = 0; i < CAM_ISP_STREAM_CFG_MAX; i++) {
		c_ctx->mapping_table.virtual_rdi[i] = 0;
		c_ctx->mapping_table.acquired_rdi[i] = 0;
	}

	kfree(c_ctx->res_list_ife_out);
	c_ctx->pri_rdi_out_res = g_ife_hw_mgr.isp_caps.max_vfe_out_res_type;
	memset(c_ctx->vfe_out_map, 0, sizeof(uint8_t) * max_ife_out_res);
	c_ctx->res_list_ife_out = NULL;
	if (c_ctx->ctx_type == CAM_IFE_CTX_TYPE_SFE) {
		kfree(c_ctx->res_list_sfe_out);
		c_ctx->res_list_sfe_out = NULL;
	...
	struct cam_hw_prepare_update_args     *prepare)
{
	...
	if ((prepare->num_hw_update_entries + 1) >=
		...
		CAM_ERR(CAM_ISP, "Insufficient HW entries :%d max:%d ctx_idx: %u",
			...
			prepare->max_hw_update_entries, c_ctx->ctx_index);
		rc = -EINVAL;
		goto end;
	}
	...
	case CAM_UBWC_CFG_VERSION_1:
		CAM_DBG(CAM_ISP, "ctx_idx: %u num_ports= %d",
			c_ctx->ctx_index, ubwc_config->num_ports);
		...
		for (i = 0; i < ubwc_config->num_ports; i++) {
			...
			CAM_DBG(CAM_ISP, "UBWC config idx %d, port_type=%d ctx_idx: %u", i,
				ubwc_plane_cfg->port_type, c_ctx->ctx_index);
			...
			if ((kmd_buf_info->used_bytes
				...
				"no free kmd memory for base=%d bytes_used=%u buf_size=%u ctx_idx: %u",
					...
					kmd_buf_info->size, c_ctx->ctx_index);
				rc = -ENOMEM;
				goto end;
			}
			...
			cmd_buf_addr = kmd_buf_info->cpu_addr +
				...
				total_used_bytes/4;
			hw_mgr_res = &c_ctx->res_list_ife_out[c_ctx->vfe_out_map[res_id_out]];

			if (!hw_mgr_res) {
				CAM_ERR(CAM_ISP, "Invalid hw_mgr_res, ctx_idx: %u", c_ctx->ctx_index);
				rc = -EINVAL;
				goto end;
			}
	...
	default:
		CAM_ERR(CAM_ISP, "Invalid UBWC API Version %d ctx_idx: %u",
			ubwc_config->api_version, c_ctx->ctx_index);
		rc = -EINVAL;
		break;
	}
end:
	...
	struct cam_hw_prepare_update_args     *prepare)
{
	...
	CAM_DBG(CAM_ISP, "ctx_idx: %u num_ports= %d", c_ctx->ctx_index, ubwc_config->num_ports);
	...
	struct cam_hw_prepare_update_args           *prepare)
{
	...
	c_ctx = ((struct cam_ife_hw_mgr_ctx *)
			prepare->ctxt_to_hw_map)->concr_ctx;
	ife_scratch_config = c_ctx->scratch_buf_info.ife_scratch_config;
	...
	for (i = 0; i < scratch_config->num_ports; i++) {
		...
		CAM_DBG(CAM_ISP, "scratch config idx: %d res: 0x%x ctx_idx: %u",
			i, buffer_info->resource_type, c_ctx->ctx_index);

		ife_out_res = &c_ctx->res_list_ife_out[c_ctx->vfe_out_map[res_id_out]];
		if (!ife_out_res->hw_res[0]) {
			CAM_ERR(CAM_ISP,
				"IFE rsrc_type: 0x%x not acquired, failing scratch config, ctx_idx: %u",
				buffer_info->resource_type, c_ctx->ctx_index);
			return -EINVAL;
		}
	...
	struct cam_hw_prepare_update_args           *prepare)
{
	...
	for (i = 0; i < scratch_config->num_ports; i++) {
		...
		sfe_out_res = &c_ctx->res_list_sfe_out[c_ctx->sfe_out_map[res_id_out]];
		...
		port_info = &c_ctx->scratch_buf_info.sfe_scratch_config->buf_info[res_id_out];
		rc = cam_isp_scratch_buf_update_util(buffer_info, port_info);
		if (rc)
			goto end;
	...
	if (c_ctx->scratch_buf_info.sfe_scratch_config->num_config !=
		c_ctx->scratch_buf_info.num_fetches) {
		CAM_ERR(CAM_ISP,
			"Mismatch in number of scratch buffers provided: %u expected: %u ctx_idx: %u",
			c_ctx->scratch_buf_info.sfe_scratch_config->num_config,
			c_ctx->scratch_buf_info.num_fetches, c_ctx->ctx_index);
		rc = -EINVAL;
	}
	...
	struct cam_ife_hw_mgr_ctx  *ctx, uint32_t hw_idx)
{
	...
	if (!supported_sc_idx) {
		CAM_DBG(CAM_ISP, "Unsupported SCID for SFE %u exp_type %u ctx_idx: %u",
			hw_idx, exp_type, c_ctx->ctx_index);
		goto end;
	}
	...
	if (scid_idx >= CAM_LLCC_MAX) {
		...
		CAM_DBG(CAM_ISP, "SFE %u Exp type %u SCID index %d use_large %d ctx_idx: %u",
			hw_idx, exp_type, scid_idx, use_large, c_ctx->ctx_index);
	}
	...
	struct cam_hw_prepare_update_args   *prepare)
{
	...
	c_ctx = ((struct cam_ife_hw_mgr_ctx *)
			prepare->ctxt_to_hw_map)->concr_ctx;
	hw_mgr = c_ctx->hw_mgr;
	memset(c_ctx->flags.sys_cache_usage, false, sizeof(c_ctx->flags.sys_cache_usage));

	if (!hw_mgr->num_caches_found) {
		CAM_DBG(CAM_ISP, "No caches found during probe, ctx_idx: %u", c_ctx->ctx_index);
		return 0;
	}

	if (!exp_config->num_ports) {
		CAM_ERR(CAM_ISP, "Invalid number of ports: %d ctx_idx: %u",
			exp_config->num_ports, c_ctx->ctx_index);
		return -EINVAL;
	}
	...
	for (i = 0; i < exp_config->num_ports; i++) {
		...
		if (!rc) {
			CAM_ERR(CAM_ISP,
				...
				order_cfg->res_type, c_ctx->ctx_index);
			return -EINVAL;
		}
		...
		if ((order_cfg->res_type - CAM_ISP_SFE_OUT_RES_RDI_0) >=
			...
			CAM_DBG(CAM_ISP,
				...
				order_cfg->res_type, c_ctx->scratch_buf_info.num_fetches,
				i, exp_order_max, c_ctx->ctx_index);
			continue;
		}
		...
		if (wm_rm_cache_cfg.use_cache) {
			...
			if (wm_rm_cache_cfg.scid <= 0)
				goto end;
			c_ctx->flags.sys_cache_usage[sc_idx] = true;
		}
		...
		if (res_id_out >= max_sfe_out_res) {
			CAM_ERR_RATE_LIMIT(CAM_ISP,
				...
				res_id_out, max_sfe_out_res, c_ctx->ctx_index);
			return -EINVAL;
		}

		hw_mgr_res = &c_ctx->res_list_sfe_out[c_ctx->sfe_out_map[res_id_out]];
		for (j = 0; j < CAM_ISP_HW_SPLIT_MAX; j++) {
		...
		if (rc || !send_config) {
			CAM_ERR(CAM_ISP,
				...
				order_cfg->res_type, base_idx, send_config, rc, c_ctx->ctx_index);
			return -EINVAL;
		}
		...
		list_for_each_entry_safe(hw_mgr_res,
				tmp, &c_ctx->res_list_ife_in_rd, list) {
			for (j = 0; j < CAM_ISP_HW_SPLIT_MAX; j++) {
				if (!hw_mgr_res->hw_res[j])
					continue;
		...
		if (rc || !send_config) {
			CAM_ERR(CAM_ISP,
				...
				res_id_in, base_idx, send_config, rc, c_ctx->ctx_index);
			return -EINVAL;
		}
	...
	struct cam_hw_prepare_update_args     *prepare)
{
	...
	if (prepare->num_hw_update_entries + 1 >=
		...
		CAM_ERR(CAM_ISP, "Insufficient HW entries :%d, ctx_idx: %u",
			prepare->num_hw_update_entries, c_ctx->ctx_index);
		return -EINVAL;
	}
	...
	list_for_each_entry(hw_mgr_res,  &c_ctx->res_list_ife_in_rd, list) {
		if ((kmd_buf_info->used_bytes
			...
			CAM_ERR(CAM_ISP,
				...
				blob_info->base_info->idx, c_ctx->ctx_index);
				rc = -ENOMEM;
				return rc;
		}
		...
		if ((c_ctx->ctx_config &
			...
			c_ctx->scratch_buf_info.sfe_scratch_config->updated_num_exp))
			enable = false;
		else
			enable = true;
	...
	enum cam_isp_hw_type                   hw_type)
{
	...
	CAM_DBG(CAM_ISP, "ctx_idx: %u num_ports= %d",
		c_ctx->ctx_index, hfr_config->num_ports);
	...
	for (i = 0; i < hfr_config->num_ports; i++) {
		...
		if (c_ctx->flags.per_port_en) {
			out_port_res_type =
				cam_ife_hw_mgr_get_virtual_mapping_out_port(prepare->ctxt_to_hw_map,
		...
		CAM_DBG(CAM_ISP, "type %d hfr config idx %d, type=%d ctx_idx: %u",
			hw_type, i, res_id_out, c_ctx->ctx_index);

		if (res_id_out >= out_max) {
			CAM_ERR(CAM_ISP, "invalid out restype:%x, ctx_idx: %u out_max:%x",
				port_hfr_config->resource_type, c_ctx->ctx_index, out_max);
			return -EINVAL;
		}
		...
		if (rc < 0) {
			CAM_ERR(CAM_ISP,
				...
				blob_info->base_info->idx, bytes_used,
				port_hfr_config->resource_type, hw_type, c_ctx->ctx_index);
			return rc;
		}
	...
	struct cam_hw_prepare_update_args     *prepare)
{
	...
	CAM_INFO(CAM_ISP,
		"csid mup value=%u, ctx_idx: %u", mup_config->mup, c_ctx->ctx_index);
	...
	struct cam_hw_prepare_update_args     *prepare)
{
	...
	CAM_DBG(CAM_ISP,
		"csid clk=%llu, ctx_idx: %u", clock_config->csid_clock, c_ctx->ctx_index);
	...
	list_for_each_entry(hw_mgr_res, &c_ctx->res_list_ife_csid, list) {
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			if (hw_intf && hw_intf->hw_ops.process_cmd) {
				...
				CAM_ERR(CAM_ISP, "NULL hw_intf! ctx_idx: %u", c_ctx->ctx_index);
		}
	}
	...
	struct cam_hw_prepare_update_args     *prepare)
{
	...
	CAM_DBG(CAM_ISP,
		"csid binning=%d, ctx_idx: %u", qcfa_config->csid_binning, c_ctx->ctx_index);
		...
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			if (hw_intf && hw_intf->hw_ops.process_cmd) {
				...
				CAM_DBG(CAM_ISP, "i= %d QCFA binning=%d\n ctx_idx: %u",
					i, csid_qcfa_upd_args.qcfa_binning, c_ctx->ctx_index);
	...
	struct cam_ife_hw_mgr_ctx             *ctx)
{
	...
	CAM_DBG(CAM_PERF,
		...
		clock_config->right_pix_hz, c_ctx->ctx_index);
		...
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			if (hw_intf && hw_intf->hw_ops.process_cmd) {
				...
				CAM_DBG(CAM_PERF,
					...
					hw_mgr_res->res_id, i, clk_rate, c_ctx->ctx_index);
				...
				if (rc) {
					CAM_ERR(CAM_PERF,
						...
						hw_intf->hw_idx, clk_rate, rc, c_ctx->ctx_index);
					goto end;
				}
			} else
				CAM_WARN(CAM_ISP, "NULL hw_intf! ctx_idx: %u", c_ctx->ctx_index);
	...
	struct cam_isp_vfe_wm_config          *wm_config)
{
	...
	list_for_each_entry(sfe_rd_res, &c_ctx->res_list_ife_in_rd,
		list) {
	...
	if (!found) {
		CAM_ERR(CAM_ISP,
			...
			wm_config->port_type, c_ctx->ctx_index);
		return -EINVAL;
	}
	...
	struct cam_isp_vfe_wm_config          *wm_config)
{
	...
	if (res_id >= CAM_SFE_FE_RDI_NUM_MAX) {
		CAM_ERR_RATE_LIMIT(CAM_ISP, "res_id: %d exceeds max size: %d ctx_idx: %u",
			res_id, CAM_SFE_FE_RDI_NUM_MAX, c_ctx->ctx_index);
		return -EINVAL;
	}

	if (!c_ctx->scratch_buf_info.sfe_scratch_config->buf_info[res_id].config_done) {
		CAM_ERR(CAM_ISP,
			"Scratch buffer not configured on ctx: %u for res: %u ctx_idx: %u",
			c_ctx->ctx_index, res_id, c_ctx->ctx_index);
		return -EINVAL;
	}

	port_info = &c_ctx->scratch_buf_info.sfe_scratch_config->buf_info[res_id];
	port_info->offset = wm_config->offset;

	CAM_DBG(CAM_ISP, "Scratch addr: 0x%x offset: %u updated for: 0x%x ctx_idx: %u",
		port_info->io_addr, port_info->offset, wm_config->port_type, c_ctx->ctx_index);
	...
	enum cam_isp_hw_type                   hw_type)
{
	...
	for (i = 0; i < vfe_out_config->num_ports; i++) {
		...
		CAM_DBG(CAM_ISP, "%s out config idx: %d port: 0x%x, ctx_idx: %u",
			...
			i, wm_config->port_type, c_ctx->ctx_index);
		...
		if (hw_type == CAM_ISP_HW_TYPE_SFE) {
			...
			isp_out_res = &c_ctx->res_list_sfe_out[c_ctx->sfe_out_map[res_id_out]];
		} else {
			isp_out_res = &c_ctx->res_list_ife_out[c_ctx->vfe_out_map[res_id_out]];
		}
	...
	struct cam_hw_prepare_update_args     *prepare)
{
	...
	if (list_empty(&c_ctx->res_list_ife_src)) {
		CAM_ERR(CAM_ISP, "Mux List empty, ctx_idx: %u", c_ctx->ctx_index);
		return -ENODEV;
	}
	...
	list_for_each_entry(hw_mgr_res,
		...
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			if (hw_intf && hw_intf->hw_ops.process_cmd) {
				...
				if (rc)
					CAM_ERR(CAM_ISP,
					"blanking update failed, ctx_idx: %u", c_ctx->ctx_index);
			}
		}
	}
	...
	enum cam_isp_hw_type                       hw_type)
{
	...
	for (i = 0; i < bw_limit_cfg->num_ports; i++) {
		...
		CAM_DBG(CAM_ISP,
			...
			wm_bw_limit_cfg->counter_limit[1], c_ctx->ctx_index);
		...
		if (rc < 0) {
			CAM_ERR(CAM_ISP,
				...
				blob_info->base_info->idx, bytes_used, rc, c_ctx->ctx_index);
			return rc;
		}
	...
	uint32_t                           blob_type)
{
	...
	if (csid_path_id == CAM_IFE_PIX_PATH_RES_MAX) {
		CAM_ERR(CAM_ISP, "Invalid res_id %u, ctx_idx: %u",
			isp_lcr_cfg->rdi_lcr_cfg->res_id, c_ctx->ctx_index);
		return -EINVAL;
	}
	...
	CAM_DBG(CAM_ISP, "Ctx %d res: %u lcr %u id %u ctx_type %u", c_ctx->ctx_index, ife_res_id,
		isp_lcr_cfg->rdi_lcr_cfg->res_id, blob_info->base_info->idx, c_ctx->ctx_type);
	list_for_each_entry(hw_mgr_res, &c_ctx->res_list_ife_src, list) {
		if (hw_mgr_res->res_type == CAM_ISP_RESOURCE_UNINT)
			continue;
			...
			if ((c_ctx->ctx_type == CAM_IFE_CTX_TYPE_SFE &&
				...
				if (rc)
					CAM_ERR(CAM_ISP,
						...
						c_ctx->ctx_index, ife_res_id,
						isp_lcr_cfg->rdi_lcr_cfg->res_id,
						blob_info->base_info->idx, c_ctx->ctx_type, rc);
				goto end;
			}
		}
	}
	...
	uint32_t                           blob_type)
{
	...
	csid_res_id = cam_ife_hw_mgr_get_ife_csid_rdi_res_type(
			rdi_lcr_cfg->res_id);
	CAM_DBG(CAM_ISP,
		"Ctx: %u csid_res_id: %u rdi_lcr: %u sfe_shdr %u ctx_ctype %u", c_ctx->ctx_index,
		csid_res_id, rdi_lcr_cfg->res_id, c_ctx->flags.is_sfe_shdr, c_ctx->ctx_type);
	...
	list_for_each_entry(hw_mgr_res, &c_ctx->res_list_ife_csid, list) {
		...
		if (!c_ctx->flags.is_sfe_shdr && hw_mgr_res->res_id != csid_res_id)
			continue;
		...
		if (rc) {
			CAM_ERR(CAM_ISP,
				...
				c_ctx->ctx_index, csid_res_id, rdi_lcr_cfg->res_id,
				c_ctx->flags.is_sfe_shdr, c_ctx->ctx_type);
			break;
		}
	}
	...
	uint32_t                           blob_type)
{
	...
	CAM_DBG(CAM_ISP,
		...
		prepare_hw_data->packet_opcode_type, rdi_lcr_cfg->res_id, c_ctx->ctx_type,
		c_ctx->flags.is_sfe_shdr, c_ctx->flags.rdi_lcr_en, c_ctx->ctx_index);
	...
	if (prepare_hw_data->packet_opcode_type == CAM_ISP_PACKET_INIT_DEV) {
		...
		if (rc) {
			CAM_ERR(CAM_ISP,
				...
				c_ctx->ctx_index, rdi_lcr_cfg->res_id,
				c_ctx->ctx_type, c_ctx->ctx_index);
			return rc;
		}

		isp_cfg_args.is_init = true;
		c_ctx->flags.rdi_lcr_en = true;
	} else if (!c_ctx->flags.rdi_lcr_en || !c_ctx->flags.is_sfe_shdr) {
		/*
...
static int cam_isp_validate_scratch_buffer_blob(
	...
	struct cam_isp_sfe_init_scratch_buf_config *scratch_config)
{
	struct cam_ife_hw_concrete_ctx        *c_ctx = ife_mgr_ctx->concr_ctx;

	if (!(c_ctx->flags.is_sfe_fs ||
		c_ctx->flags.is_sfe_shdr)) {
		CAM_ERR(CAM_ISP,
			"Not SFE sHDR/FS context: %u scratch buf blob not supported, ctx_idx: %u",
			c_ctx->ctx_index, c_ctx->ctx_index);
		return -EINVAL;
	}
	...
	if ((scratch_config->num_ports >
		...
		CAM_ERR(CAM_ISP,
			...
			scratch_config->num_ports, c_ctx->ctx_index);
		return -EINVAL;
	}
	...
	if (blob_size <
		...
		CAM_ERR(CAM_ISP, "Invalid blob size: %u expected: %lu ctx_idx: %u",
			...
			sizeof(struct cam_isp_sfe_init_scratch_buf_config) +
			(scratch_config->num_ports - 1) *
			sizeof(struct cam_isp_sfe_scratch_buf_info), c_ctx->ctx_index);
		return -EINVAL;
	}
	...
	enum cam_isp_hw_type                   hw_type)
{
	...
	CAM_DBG(CAM_ISP,
		...
		c_ctx->ctx_index, hw_type, request_id);
	...
	default:
		CAM_ERR(CAM_ISP,
			...
			hw_type, c_ctx->ctx_index, request_id);
		return -EINVAL;
	}
	...
	struct cam_ife_hw_mgr_ctx         *ife_mgr_ctx)
{
	...
	if ((fcg_config_args->num_ch_ctx > max_fcg_ch_ctx) ||
		...
		CAM_ERR(CAM_ISP, "Invalid num of channels/contexts %u in FCG config, ctx_idx: %u",
			fcg_config_args->num_ch_ctx, c_ctx->ctx_index);
		return -EINVAL;
	}
	...
	if ((fcg_config_args->num_predictions > max_fcg_predictions) ||
		...
		CAM_ERR(CAM_ISP, "Invalid num of predictions %u in FCG config, ctx_idx: %u",
			fcg_config_args->num_predictions, c_ctx->ctx_index);
		return -EINVAL;
	}
	...
	uint32_t blob_type, uint32_t blob_size, uint8_t *blob_data)
{
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_HFR_CONFIG: {
		...
		if (hfr_config->num_ports > g_ife_hw_mgr.isp_caps.max_vfe_out_res_type ||
			...
			CAM_ERR(CAM_ISP, "Invalid num_ports %u in HFR config, ctx_idx: %u",
				hfr_config->num_ports, c_ctx->ctx_index);
			return -EINVAL;
		}
		...
		rc = cam_isp_blob_hfr_update(blob_type, blob_info,
			...
			CAM_ISP_HW_TYPE_VFE);
		if (rc)
			CAM_ERR(CAM_ISP, "HFR Update Failed, ctx_idx: %u", c_ctx->ctx_index);
	}
		break;
	case CAM_ISP_GENERIC_BLOB_TYPE_CLOCK_CONFIG: {
		...
		if (clock_config->num_rdi > CAM_IFE_RDI_NUM_MAX) {
			CAM_ERR(CAM_ISP, "Invalid num_rdi %u in clock config, ctx_idx: %u",
				clock_config->num_rdi, c_ctx->ctx_index);
			return -EINVAL;
		}
		...
		if (clock_config->num_rdi > 1) {
			if (sizeof(uint64_t) > ((UINT_MAX -
				...
				CAM_ERR(CAM_ISP,
					...
					sizeof(uint64_t), c_ctx->ctx_index);
				return -EINVAL;
			}
		}
		...
		if ((clock_config->num_rdi != 0) && (blob_size <
			...
			CAM_ERR(CAM_ISP, "Invalid blob size %u expected %lu ctx_idx: %u",
				...
				(clock_config->num_rdi + 2), c_ctx->ctx_index);
			return -EINVAL;
		}
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_BW_CONFIG: {
		...
		if (bw_config_u->num_rdi > CAM_IFE_RDI_NUM_MAX || !bw_config_u->num_rdi) {
			CAM_ERR(CAM_ISP, "Invalid num_rdi %u in bw config, ctx_idx: %u",
				bw_config_u->num_rdi, c_ctx->ctx_index);
			return -EINVAL;
		}
		...
		if (bw_config->num_rdi > 1) {
			if (sizeof(struct cam_isp_bw_vote) > ((UINT_MAX -
				...
				CAM_ERR(CAM_ISP,
					...
					bw_config->num_rdi,
					sizeof(struct cam_isp_bw_vote), c_ctx->ctx_index);
				cam_common_mem_free(bw_config);
				return -EINVAL;
			}
		}
		...
		if (!prepare || !prepare->priv ||
			...
			CAM_ERR(CAM_ISP, "Invalid inputs usage type %d, ctx_idx: %u",
				bw_config->usage_type, c_ctx->ctx_index);
			cam_common_mem_free(bw_config);
			return -EINVAL;
		}
		...
		c_ctx->bw_config_version = CAM_ISP_BW_CONFIG_V1;
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_BW_CONFIG_V2: {
		...
		if (bw_config_u->num_paths > CAM_ISP_MAX_PER_PATH_VOTES ||
			...
			CAM_ERR(CAM_ISP, "Invalid num paths %d ctx_idx: %u",
				bw_config_u->num_paths, c_ctx->ctx_index);
			return -EINVAL;
		}
		...
		if (bw_config->num_paths > 1) {
			if (sizeof(struct cam_axi_per_path_bw_vote) >
				...
				CAM_ERR(CAM_ISP,
					...
					bw_config->num_paths - 1,
					sizeof(
					struct cam_axi_per_path_bw_vote), c_ctx->ctx_index);
				cam_common_mem_free(bw_config);
				return -EINVAL;
			}
		}
		...
		if ((bw_config->num_paths != 0) && (blob_size <
			...
			CAM_ERR(CAM_ISP,
				...
				sizeof(struct cam_isp_bw_config_v2),
				sizeof(struct cam_axi_per_path_bw_vote), c_ctx->ctx_index);
			cam_common_mem_free(bw_config);
			return -EINVAL;
		}
		...
		c_ctx->bw_config_version = CAM_ISP_BW_CONFIG_V2;
		prepare_hw_data->bw_clk_config.bw_config_valid = true;
		cam_common_mem_free(bw_config);
	}
		break;
	case CAM_ISP_GENERIC_BLOB_TYPE_BW_CONFIG_V3: {
		...
		if ((bw_config->num_paths != 0) && (blob_size <
			...
			CAM_ERR(CAM_ISP,
				...
				blob_size, bw_config->num_paths,
				sizeof(struct cam_isp_bw_config_v3),
				sizeof(struct cam_axi_per_path_bw_vote_v2), c_ctx->ctx_index);
			cam_common_mem_free(bw_config);
			return -EINVAL;
		}
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_UBWC_CONFIG: {
		...
		if (ubwc_config->num_ports > CAM_VFE_MAX_UBWC_PORTS ||
			...
			CAM_ERR(CAM_ISP, "Invalid num_ports %u in ubwc config, ctx_idx: %u",
				ubwc_config->num_ports, c_ctx->ctx_index);
			return -EINVAL;
		}
		...
		if (blob_size < (sizeof(struct cam_ubwc_config) +
			...
			CAM_ERR(CAM_ISP, "Invalid blob_size %u expected %lu ctx_idx: %u",
				...
				(ubwc_config->num_ports - 1) *
				sizeof(struct cam_ubwc_plane_cfg_v1) * 2, c_ctx->ctx_index);
			return -EINVAL;
		}
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_UBWC_CONFIG_V2: {
		...
		if (blob_size < (sizeof(struct cam_ubwc_config_v2) +
			...
			CAM_ERR(CAM_ISP, "Invalid blob_size %u expected %lu ctx_idx: %u",
				...
				(ubwc_config->num_ports - 1) *
				sizeof(struct cam_ubwc_plane_cfg_v2) * 2, c_ctx->ctx_index);
			return -EINVAL;
		}
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_CSID_CLOCK_CONFIG: {
		...
		if (blob_size < sizeof(struct cam_isp_csid_clock_config)) {
			CAM_ERR(CAM_ISP, "Invalid blob size %u expected %lu ctx_idx: %u",
				blob_size,
				sizeof(struct cam_isp_csid_clock_config), c_ctx->ctx_index);
			return -EINVAL;
		}
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_CSID_QCFA_CONFIG: {
		...
		if (blob_size < sizeof(struct cam_isp_csid_qcfa_config)) {
			CAM_ERR(CAM_ISP,
				...
				blob_size,
				sizeof(struct cam_isp_csid_qcfa_config), c_ctx->ctx_index);
			return -EINVAL;
		}
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_FE_CONFIG: {
		...
		if (blob_size < sizeof(struct cam_fe_config)) {
			CAM_ERR(CAM_ISP, "Invalid blob size %u expected %lu, ctx_idx: %u",
				blob_size, sizeof(struct cam_fe_config), c_ctx->ctx_index);
			return -EINVAL;
		}
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_VFE_OUT_CONFIG: {
		...
		if (blob_size < sizeof(struct cam_isp_vfe_out_config)) {
			CAM_ERR(CAM_ISP, "Invalid blob size %u, ctx_idx: %u",
				blob_size,
				sizeof(struct cam_isp_vfe_out_config), c_ctx->ctx_index);
			return -EINVAL;
		}
		...
		if (vfe_out_config->num_ports >  max_ife_out_res ||
			...
			CAM_ERR(CAM_ISP,
				...
				vfe_out_config->num_ports, c_ctx->ctx_index);
			return -EINVAL;
		}
		...
		if (blob_size < (sizeof(struct cam_isp_vfe_out_config) +
			...
			CAM_ERR(CAM_ISP, "Invalid blob size %u expected %lu ctx_idx: %u",
				...
				+ (vfe_out_config->num_ports - 1) *
				sizeof(struct cam_isp_vfe_wm_config), c_ctx->ctx_index);
			return -EINVAL;
		}
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_DYNAMIC_MODE_SWITCH: {
		...
		if (blob_size < sizeof(struct cam_isp_mode_switch_info)) {
			CAM_ERR(CAM_ISP, "Invalid blob size %u expected %lu ctx_idx: %u",
				blob_size,
				sizeof(struct cam_isp_mode_switch_info), c_ctx->ctx_index);
			return -EINVAL;
		}
		...
		if (rc)
			CAM_ERR(CAM_ISP, "MUP Update Failed, ctx_idx: %u", c_ctx->ctx_index);
	}
		break;
		...
		if (prepare_hw_data->packet_opcode_type !=
			...
			CAM_ERR(CAM_ISP,
				...
				prepare_hw_data->packet_opcode_type,
				 prepare->packet->header.request_id, c_ctx->ctx_index);
			return -EINVAL;
		}
		...
		if (rc) {
			CAM_ERR(CAM_ISP,
				"CSID Init config failed for req: %llu rc: %d ctx_idx: %u",
				 prepare->packet->header.request_id, rc, c_ctx->ctx_index);
			break;
		}
		...
		if (rc)
			CAM_ERR(CAM_ISP,
				"RDI LCR config failed for res %u, ctx_idx: %u",
				 lcr_rdi_config->res_id, c_ctx->ctx_index);
...

{
	...
	if (!hw_mgr_res) {
		CAM_ERR(CAM_ISP, "Ctx:%d invalid res", c_ctx->ctx_index);
		return -EINVAL;
	}
	...
	CAM_DBG(CAM_ISP, "Ctx:%d IPP SRC mask 0x%x IPP DST mask 0x%x", c_ctx->ctx_index,
	...
	uint32_t blob_type, uint32_t blob_size, uint8_t *blob_data)
{
	...
	c_ctx = ((struct cam_ife_hw_mgr_ctx *)
			prepare->ctxt_to_hw_map)->concr_ctx;
	CAM_DBG(CAM_ISP, "Context[%pK][%u] blob_type=%d, blob_size=%d",
		ife_mgr_ctx, c_ctx->ctx_index, blob_type, blob_size);
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_SFE_OUT_CONFIG: {
		...
		if (vfe_out_config->num_ports > max_sfe_out_res ||
			...
			CAM_ERR(CAM_ISP,
				...
				vfe_out_config->num_ports,
				max_sfe_out_res, c_ctx->ctx_index);
			return -EINVAL;
		}
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_SFE_HFR_CONFIG: {
		...
		if (blob_size < (sizeof(struct cam_isp_resource_hfr_config) +
			...
			CAM_ERR(CAM_ISP, "Invalid blob size %u expected %lu, ctx_idx: %u",
				...
				sizeof(struct cam_isp_resource_hfr_config) +
				(hfr_config->num_ports - 1) *
				sizeof(struct cam_isp_port_hfr_config), c_ctx->ctx_index);
			return -EINVAL;
		}
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_SFE_CORE_CONFIG: {
		...
		if (blob_size < sizeof(struct cam_isp_sfe_core_config)) {
			CAM_ERR(CAM_ISP,
				...
				sizeof(struct cam_isp_sfe_core_config), c_ctx->ctx_index);
			return -EINVAL;
		}
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_SFE_EXP_ORDER_CFG: {
		...
		if (blob_size <
			...
			CAM_ERR(CAM_ISP, "Invalid blob size: %u expected: %lu ctx_idx: %u",
				...
				(exp_config->num_ports - 1) *
				sizeof(
				struct cam_isp_sfe_wm_exp_order_config), c_ctx->ctx_index);
			return -EINVAL;
		}
	...
	bool default_settings)
{
	...
	if (res_idx >= c_ctx->scratch_buf_info.num_fetches)
		return true;

	if (default_settings)
		curr_num_exp = c_ctx->curr_num_exp;
	else
		curr_num_exp = c_ctx->scratch_buf_info.sfe_scratch_config->updated_num_exp;
	...
	struct cam_ife_hw_mgr_ctx            *ctx)
{
	...
	CAM_DBG(CAM_ISP, "ctx_idx: %u num_ports: %u",
		c_ctx->ctx_index, c_ctx->scratch_buf_info.sfe_scratch_config->num_config);

	/* Update RDI WMs */
	for (i = 0; i < CAM_SFE_FE_RDI_NUM_MAX; i++) {
		if (c_ctx->sfe_out_map[i] == 0xff)
			continue;
		hw_mgr_res = &res_list_isp_out[c_ctx->sfe_out_map[i]];
		for (j = 0; j < CAM_ISP_HW_SPLIT_MAX; j++) {
			if (!hw_mgr_res->hw_res[j])
				continue;
			...
			CAM_DBG(CAM_ISP, "WM res_id: 0x%x idx: %u io_addr: %pK, ctx_idx: %u",
				...
				(res_id - CAM_ISP_SFE_OUT_RES_RDI_0),
				buf_info->io_addr, c_ctx->ctx_index);
	...
	struct cam_ife_hw_mgr_ctx            *ctx)
{
	...
	for (i = 0; i < c_ctx->scratch_buf_info.ife_scratch_config->num_config; i++) {
		/*
		...
		res_id = c_ctx->scratch_buf_info.ife_scratch_config->buf_info[i].res_id & 0xFF;

		hw_mgr_res = &c_ctx->res_list_ife_out[c_ctx->vfe_out_map[res_id]];
		for (j = 0; j < CAM_ISP_HW_SPLIT_MAX; j++) {
			if (!hw_mgr_res->hw_res[j])
				continue;
			...
			cpu_addr = kmd_buf_info->cpu_addr +
				kmd_buf_info->used_bytes / 4 + io_cfg_used_bytes / 4;
			buf_info = &c_ctx->scratch_buf_info.ife_scratch_config->buf_info[i];
			CAM_DBG(CAM_ISP, "WM res_id: 0x%x io_addr: %pK, ctx_idx: %u",
				hw_mgr_res->hw_res[j]->res_id, buf_info->io_addr, c_ctx->ctx_index);
	...
	struct cam_kmd_buf_info *kmd_buf)
{
	...
	list_for_each_entry(hw_mgr_res, &c_ctx->res_list_ife_csid, list) {
		...
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			CAM_DBG(CAM_ISP,
				...
				res->res_id, res->hw_intf->hw_idx, c_ctx->ctx_index);
		}
	}
	...
	for (i = 0; i < CAM_IFE_CSID_HW_NUM_MAX; i++) {
		...
		CAM_DBG(CAM_ISP, "Ctx:%u Change base added for num_res %d",
			c_ctx->ctx_index, rup_args[i].num_res);
	...
	struct cam_kmd_buf_info *kmd_buf)
{
	...
	for (i = 0; i < c_ctx->num_base; i++) {
		...
		change_base_info.base_idx = c_ctx->base[i].idx;
		change_base_info.cdm_id = c_ctx->cdm_id;

		/* Add change base */
		if (!c_ctx->flags.internal_cdm) {
			rc = cam_isp_add_change_base(prepare,
				&c_ctx->res_list_ife_src,
				&change_base_info, kmd_buf);
			...
			if (rc) {
				CAM_ERR(CAM_ISP,
					...
					i, c_ctx->base[i].idx, rc, c_ctx->ctx_index);
				break;
			}
		...
		rc = cam_isp_add_reg_update(prepare,
			&c_ctx->res_list_ife_src,
			c_ctx->base[i].idx, kmd_buf,
			!c_ctx->flags.internal_cdm, NULL);
		if (rc) {
			CAM_ERR(CAM_ISP,
		...
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			if (i == CAM_ISP_HW_SPLIT_RIGHT) {
				CAM_ERR(CAM_ISP,
					...
					hw_mgr_res->hw_res[i]->res_id, c_ctx->ctx_index);
				return -EINVAL;
			}
...
add_cmds:
	...
	if (is_found) {
		...
		rc = cam_isp_add_change_base(prepare, &c_ctx->res_list_ife_src,
			&change_base_info, kmd_buf_info);
		if (rc)
			return rc;
	...
	uint32_t                                 base_idx)
{
	...
	if (c_ctx->base[base_idx].hw_type == CAM_ISP_HW_TYPE_SFE) {
		res_list = &c_ctx->res_list_sfe_src;
	} else if (c_ctx->base[base_idx].hw_type == CAM_ISP_HW_TYPE_VFE) {
		res_list = &c_ctx->res_list_ife_src;
	} else if (c_ctx->base[base_idx].hw_type == CAM_ISP_HW_TYPE_CSID) {
		if (!cmd_buf_count->csid_cnt && !prepare_hw_data->irq_comp_cfg_valid)
			return rc;
		res_list = &c_ctx->res_list_ife_csid;
	} else {
		CAM_ERR(CAM_ISP,
			"Invalid hw_type=%d, ctx_idx: %u",
			c_ctx->base[base_idx].hw_type, c_ctx->ctx_index);
		return -EINVAL;
	}

	if (!c_ctx->flags.internal_cdm) {
		change_base_info.base_idx = c_ctx->base[base_idx].idx;
		change_base_info.cdm_id = c_ctx->cdm_id;
		rc = cam_isp_add_change_base(prepare,
			res_list,
			&change_base_info, kmd_buf);
		if (rc) {
			CAM_ERR(CAM_ISP,
				...
				base_idx, c_ctx->base[base_idx].split_id,
				c_ctx->base[base_idx].hw_type, c_ctx->ctx_index);
			return rc;
		}
		...
		CAM_DBG(CAM_ISP,
			...
			c_ctx->base[base_idx].hw_type, c_ctx->base[base_idx].idx,
			c_ctx->cdm_id, c_ctx->ctx_index);
	}
	...
	if (c_ctx->base[base_idx].hw_type == CAM_ISP_HW_TYPE_SFE) {
		rc = cam_sfe_add_command_buffers(
			prepare, kmd_buf, &c_ctx->base[base_idx],
			cam_sfe_packet_generic_blob_handler,
			c_ctx->res_list_sfe_out,
			c_ctx->sfe_out_map,
			CAM_ISP_SFE_OUT_RES_BASE,
			(CAM_ISP_SFE_OUT_RES_BASE +
			max_sfe_out_res));
...
add_cmd_err:
	CAM_ERR(CAM_ISP,
		...
		base_idx, c_ctx->base[base_idx].split_id, rc,
		c_ctx->base[base_idx].hw_type, c_ctx->ctx_index);
end:
	return rc;
}
	...
	struct cam_isp_check_io_cfg_for_scratch *check_for_scratch)
{
	...
	if (c_ctx->scratch_buf_info.ife_scratch_config->num_config) {
		int i;

		check_for_scratch->validate_for_ife = true;
		for (i = 0; i < c_ctx->scratch_buf_info.ife_scratch_config->num_config; i++) {
			check_for_scratch->ife_scratch_res_info.ife_scratch_resources[i] =
				c_ctx->scratch_buf_info.ife_scratch_config->buf_info[i].res_id;
	...
	struct cam_isp_sfe_scratch_buf_res_info *sfe_res_info)
{
	...
	if ((sfe_res_info->sfe_rdi_cfg_mask) !=
		((1 << c_ctx->scratch_buf_info.num_fetches) - 1)) {
		...
		if (opcode_type == CAM_ISP_PACKET_UPDATE_DEV) {
			CAM_DBG(CAM_ISP,
				...
				((1 << c_ctx->scratch_buf_info.num_fetches) - 1),
				sfe_res_info->sfe_rdi_cfg_mask, c_ctx->ctx_index);
			rc = cam_isp_sfe_add_scratch_buffer_cfg(
				c_ctx->base[base_idx].idx, sfe_res_info->sfe_rdi_cfg_mask,
				prepare, kmd_buf, c_ctx->res_list_sfe_out,
				&c_ctx->res_list_ife_in_rd, ctx);
			if (rc)
				goto end;
		} else if (opcode_type == CAM_ISP_PACKET_INIT_DEV) {
			/* For INIT packets update mask which is applied at streamon */
			c_ctx->scratch_buf_info.sfe_scratch_config->streamon_buf_mask =
		...
		if (opcode_type == CAM_ISP_PACKET_INIT_DEV)
			c_ctx->scratch_buf_info.sfe_scratch_config->skip_scratch_cfg_streamon
				= true;
	}
	...
	if ((ife_res_info->ife_scratch_cfg_mask) !=
		...
		if (opcode_type == CAM_ISP_PACKET_UPDATE_DEV) {
			CAM_DBG(CAM_ISP,
				...
				((1 << ife_res_info->num_ports) - 1),
				ife_res_info->ife_scratch_cfg_mask, c_ctx->ctx_index);
			rc = cam_isp_ife_add_scratch_buffer_cfg(
				c_ctx->base[base_idx].idx,
				ife_res_info->ife_scratch_cfg_mask, prepare,
				kmd_buf, ctx);
			if (rc)
				goto end;
		} else if (opcode_type == CAM_ISP_PACKET_INIT_DEV) {
			c_ctx->scratch_buf_info.ife_scratch_config->streamon_buf_mask =
				ife_res_info->ife_scratch_cfg_mask;
		}
	} else {
		if (opcode_type == CAM_ISP_PACKET_INIT_DEV)
			c_ctx->scratch_buf_info.ife_scratch_config->skip_scratch_cfg_streamon
				= true;
	}
	...
	void *prepare_hw_update_args)
{
	...
	ife_mgr_ctx = (struct cam_ife_hw_mgr_ctx *)prepare->ctxt_to_hw_map;
	c_ctx = ife_mgr_ctx->concr_ctx;

	hw_mgr = (struct cam_ife_hw_mgr *)hw_mgr_priv;

	CAM_DBG(CAM_REQ, "ctx[%pK][%u] Enter for req_id %lld",
		c_ctx, c_ctx->ctx_index, prepare->packet->header.request_id);
	...
	if (rc) {
		CAM_ERR(CAM_ISP, "Patch ISP packet failed. ctx_idx: %u", c_ctx->ctx_index);
		return rc;
	}
	...
	prepare_hw_data->per_port_enable = c_ctx->flags.per_port_en;

	if (ife_mgr_ctx->virtual_rdi_mapping_cb) {
	...
	if (c_ctx->ctx_type != CAM_IFE_CTX_TYPE_SFE)
		res_list_ife_rd_tmp = &c_ctx->res_list_ife_in_rd;

	rc = cam_isp_get_cmd_buf_count(prepare, &cmd_buf_count);

	if (rc) {
		CAM_ERR(CAM_ISP, "Invalid cmd buffers, ctx_idx: %u", c_ctx->ctx_index);
		return rc;
	}
	...
	cam_ife_hw_mgr_check_if_scratch_is_needed(ife_mgr_ctx, &check_for_scratch);
	...
	for (i = 0; i < c_ctx->num_base; i++) {
		...
		io_buf_info.base = &c_ctx->base[i];
		io_buf_info.hw_intf = cam_ife_hw_mgr_get_hw_intf(&c_ctx->base[i]);
		io_buf_info.unpacker_fmt = ife_mgr_ctx->unpacker_fmt;
		io_buf_info.major_version  = c_ctx->major_version;
		...
		if (c_ctx->base[i].hw_type == CAM_ISP_HW_TYPE_VFE) {
			...
			io_buf_info.res_list_isp_out = c_ctx->res_list_ife_out;
			io_buf_info.out_map = c_ctx->vfe_out_map;
			...
			io_buf_info.res_list_in_rd = &c_ctx->res_list_ife_in_rd;
			io_buf_info.res_list_isp_out = c_ctx->res_list_sfe_out;
			io_buf_info.out_map = c_ctx->sfe_out_map;
			rc = cam_isp_add_io_buffers(&io_buf_info);
		}
		...
		if (rc) {
			CAM_ERR(CAM_ISP,
				...
				i, rc,
				(c_ctx->base[i].hw_type == CAM_ISP_HW_TYPE_SFE ? "SFE" : "IFE"),
				c_ctx->ctx_index);
			goto end;
		}
		...
		if ((check_for_scratch.validate_for_sfe) &&
			(c_ctx->base[i].hw_type == CAM_ISP_HW_TYPE_SFE) && (fill_sfe_fence)) {
			struct cam_isp_sfe_scratch_buf_res_info *sfe_res_info =
				&check_for_scratch.sfe_scratch_res_info;

			rc = cam_ife_hw_mgr_sfe_scratch_buf_update(
				prepare_hw_data->packet_opcode_type,
				i, &prepare_hw_data->kmd_cmd_buff_info, prepare,
				ife_mgr_ctx, sfe_res_info);
			if (rc)
				goto end;
		}

		if ((check_for_scratch.validate_for_ife) &&
			(c_ctx->base[i].hw_type == CAM_ISP_HW_TYPE_VFE) && (fill_ife_fence)) {
			struct cam_isp_ife_scratch_buf_res_info *ife_res_info =
			...
			rc = cam_ife_hw_mgr_ife_scratch_buf_update(
				prepare_hw_data->packet_opcode_type,
				i, &prepare_hw_data->kmd_cmd_buff_info, prepare,
				ife_mgr_ctx, ife_res_info);
			if (rc)
				goto end;
		}

		/* fence map table entries need to fill only once in the loop */
		if ((c_ctx->base[i].hw_type == CAM_ISP_HW_TYPE_SFE) && fill_sfe_fence)
			fill_sfe_fence = false;
		else if ((c_ctx->base[i].hw_type == CAM_ISP_HW_TYPE_VFE) && fill_ife_fence)
		...
		if (frame_header_info.frame_header_res_id &&
			...
			CAM_DBG(CAM_ISP,
				...
				prepare_hw_data->frame_header_res_id,
				prepare_hw_data->frame_header_cpu_addr, c_ctx->ctx_index);
		}
	}
	...
	if ((c_ctx->ctx_config & CAM_IFE_CTX_CFG_FRAME_HEADER_TS) &&
		...
		(!prepare_hw_data->frame_header_res_id)) {
		CAM_ERR(CAM_ISP, "Failed to configure frame header, ctx_idx: %u", c_ctx->ctx_index);
		goto end;
	}
	 ...
	 */
	if ((prepare_hw_data->packet_opcode_type ==
		CAM_ISP_PACKET_INIT_DEV) && (!c_ctx->flags.is_offline ||
		atomic_read(&c_ctx->ctx_state) == CAM_IFE_HW_STATE_STARTING)) {
		...
		if (!c_ctx->num_reg_dump_buf) {
			c_ctx->num_reg_dump_buf = prepare->num_reg_dump_buf;

			memcpy(c_ctx->reg_dump_buf_desc,
	...
	if (hw_mgr->csid_aup_rup_en)
		rc = cam_ife_mgr_csid_add_reg_update(ife_mgr_ctx,
			prepare, &prepare_hw_data->kmd_cmd_buff_info);

	else
		rc = cam_ife_mgr_isp_add_reg_update(ife_mgr_ctx,
			prepare, &prepare_hw_data->kmd_cmd_buff_info);

	if (rc) {
		CAM_ERR(CAM_ISP, "Add RUP fail csid_aup_rup_en %d, ctx_idx: %u",
			hw_mgr->csid_aup_rup_en, c_ctx->ctx_index);
		goto end;
	}
...
offline:
	...
	if (prepare->num_out_map_entries &&
		...
		c_ctx->flags.is_offline) {
		if (c_ctx->ctx_type != CAM_IFE_CTX_TYPE_SFE)
			rc = cam_ife_hw_mgr_add_vfe_go_cmd(ife_mgr_ctx, prepare,
				&prepare_hw_data->kmd_cmd_buff_info);
		else
			rc = cam_ife_hw_mgr_add_csid_go_cmd(ife_mgr_ctx, prepare,
				&prepare_hw_data->kmd_cmd_buff_info);
		if (rc)
			CAM_ERR(CAM_ISP,
	...
	uint32_t res_id)
{
	...
	list_for_each_entry(isp_hw_res, &c_ctx->res_list_ife_in_rd, list) {
	...
	uint32_t res_id, uint32_t hw_type)
{
	...
	if (hw_type == CAM_ISP_HW_TYPE_VFE) {
		if (cam_ife_hw_mgr_is_ife_out_port(res_id)) {
			isp_hw_res = &c_ctx->res_list_ife_out[c_ctx->vfe_out_map[res_id & 0xFF]];
		...
		if (cam_ife_hw_mgr_is_sfe_out_port(res_id)) {
			isp_hw_res = &c_ctx->res_list_sfe_out[c_ctx->sfe_out_map[res_id & 0xFF]];
...
static void cam_ife_mgr_pf_dump(struct cam_ife_hw_mgr_ctx *ctx)
{
	...
	if (rc)
		CAM_ERR(CAM_ISP,
			...
			c_ctx->applied_req_id, rc, c_ctx->ctx_index);
	...
	struct cam_packet            *packet)
{
	...
	c_ctx->flags.pf_mid_found = true;
	c_ctx->pf_info.mid = get_res.mid;
	c_ctx->pf_info.out_port_id = get_res.out_res_id;
	CAM_ERR(CAM_ISP,
		"Page fault on resource id:(0x%x) ctx id:%u req id:%lld",
		get_res.out_res_id, c_ctx->ctx_index, packet->header.request_id);
}
	...
	struct cam_hw_cmd_args *hw_cmd_args)
{
	...
	if ((*ctx_found) && (c_ctx->flags.pf_mid_found))
		goto outportlog;

	/* Determine if the current context is the faulted context based on pid */
	for (i = 0; i < c_ctx->num_base; i++) {
		if (c_ctx->base[i].hw_type == CAM_ISP_HW_TYPE_VFE)
			hw_intf_data = g_ife_hw_mgr.ife_devices[c_ctx->base[i].idx];
		else if (c_ctx->base[i].hw_type == CAM_ISP_HW_TYPE_SFE)
			hw_intf_data = g_ife_hw_mgr.sfe_devices[c_ctx->base[i].idx];
		else
			continue;
			...
			if (hw_intf_data->hw_pid[j] == pf_args->pf_smmu_info->pid) {
				*ctx_found = true;
				CAM_ERR(CAM_ISP, "PF found for %s%d pid: %u ctx_idx: %u",
					c_ctx->base[i].hw_type == CAM_ISP_HW_TYPE_VFE ? "VFE" : "SFE",
					c_ctx->base[i].idx, pf_args->pf_smmu_info->pid,
					c_ctx->ctx_index);
				cam_ife_mgr_pf_dump_mid_info(hw_mgr_ctx, hw_cmd_args, hw_intf_data,
					packet);

				/* If MID found - stop hw res and dump client info */
				if (c_ctx->flags.pf_mid_found) {
					cam_ife_hw_mgr_stop_pf_hw_res(hw_mgr_ctx,
						c_ctx->pf_info.out_port_id, c_ctx->base[i].hw_type);
					cam_ife_hw_mgr_dump_bus_info(c_ctx->pf_info.out_port_id,
						hw_intf_data);
				}
				break;
			}
	...
	if (g_ife_hw_mgr.hw_pid_support && (i == c_ctx->num_base || !*ctx_found))
		CAM_INFO(CAM_ISP,
			"This context does not cause pf:pid:%d ctx_id:%u",
			pf_args->pf_smmu_info->pid, c_ctx->ctx_index);
		...
		struct cam_ife_hw_mgr_ctx *hw_mgr_ctx, int64_t req_id)
{
	...
	cam_ife_mgr_flush_in_queue(ife_hw_mgr,
		hw_mgr_ctx->ctx_idx, false, req_id);
	...
	struct cam_ife_hw_mgr_ctx *ctx)
{
	...
	for (j = 0; j < c_ctx->num_base; j++) {
		...
		list_for_each_entry(hw_mgr_res, res_list, list) {
			...
			for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
				...
				res = hw_mgr_res->hw_res[i];
				if (res->hw_intf->hw_idx != c_ctx->base[j].idx)
					continue;
				...
				CAM_DBG(CAM_ISP,
					...
					res->res_id, res->hw_intf->hw_idx, c_ctx->base[j].idx,
					c_ctx->ctx_index);
			}
		}
	}
	...
	for (i = 0; i < CAM_IFE_CSID_HW_NUM_MAX; i++) {
		...
		rup_args[i].last_applied_mup = c_ctx->current_mup;
		...
		CAM_DBG(CAM_ISP,
			...
			res->hw_intf->hw_idx, c_ctx->current_mup, c_ctx->ctx_index);
	}
	...
	struct cam_ife_hw_mgr_ctx *ctx)
{
	...
	ife_buf_info = c_ctx->scratch_buf_info.ife_scratch_config;
	res_list_ife_out = c_ctx->res_list_ife_out;

	if (c_ctx->scratch_buf_info.ife_scratch_config->skip_scratch_cfg_streamon)
		goto end;

	for (i = 0; i < ife_buf_info->num_config; i++) {
		res_id = ife_buf_info->buf_info[i].res_id & 0xFF;
		port_info = &ife_buf_info->buf_info[i];
		hw_mgr_res = &res_list_ife_out[c_ctx->vfe_out_map[res_id]];
		...
		for (j = 0; j < CAM_ISP_HW_SPLIT_MAX; j++) {
			...
			if ((is_streamon) &&
				...
				i, c_ctx->scratch_buf_info.ife_scratch_config->streamon_buf_mask))
				continue;
	...
	bool is_streamon, struct cam_ife_hw_mgr_ctx *ctx)
{
	...
	res_list_in_rd = &c_ctx->res_list_ife_in_rd;
	res_list_sfe_out = c_ctx->res_list_sfe_out;
	sfe_scratch_config = c_ctx->scratch_buf_info.sfe_scratch_config;
	...
	for (i = 0; i < CAM_SFE_FE_RDI_NUM_MAX; i++) {
		if (c_ctx->sfe_out_map[i] == 0xff)
			continue;
		hw_mgr_res = &res_list_sfe_out[c_ctx->sfe_out_map[i]];
		for (j = 0; j < CAM_ISP_HW_SPLIT_MAX; j++) {
			/* j = 1 is not valid for this use-case */
			if (!hw_mgr_res->hw_res[j])
				continue;
	...
	struct cam_isp_hw_done_event_data *done)
{
	...
	if (done->hw_type == CAM_ISP_HW_TYPE_VFE)
		res_list_isp_src = &c_ctx->res_list_ife_src;
	else if (done->hw_type == CAM_ISP_HW_TYPE_SFE)
		res_list_isp_src = &c_ctx->res_list_sfe_src;
	else {
	...
	void *dump_struct, uint8_t *addr_ptr)
{
	...
	*addr++ = c_ctx->flags.is_sfe_shdr;
	*addr++ = c_ctx->flags.is_sfe_fs;
	*addr++ = c_ctx->flags.dsp_enabled;
	*addr++ = c_ctx->flags.is_offline;
				...
				struct cam_isp_hw_cmd_args *isp_hw_cmd_args)
{
	...
	case CAM_ISP_HW_MGR_CMD_CHECK_START_OFFLINE:
		...
		cam_ife_mgr_check_start_processing(hw_mgr_priv,
			hw_mgr_ctx);
		break;
	...
	case CAM_ISP_HW_MGR_GET_PACKET_OPCODE:
		...
		isp_hw_cmd_args->u.packet_info.hw_mgr_ctx_id = c_ctx->ctx_index;
		break;
	case CAM_ISP_HW_MGR_GET_LAST_CDM_DONE:
		isp_hw_cmd_args->u.last_cdm_done =
				c_ctx->last_cdm_done_req;
		break;
	case CAM_ISP_HW_MGR_CMD_PROG_DEFAULT_CFG: {
		...
		rc = cam_ife_mgr_prog_default_settings((false || skip_rup_aup), hw_mgr_ctx);
	}
		break;
	...
	case CAM_ISP_HW_MGR_GET_BUS_COMP_GROUP:
		query_cmd = (struct cam_isp_comp_record_query *)
			isp_hw_cmd_args->cmd_data;
		memcpy(query_cmd->vfe_bus_comp_grp, c_ctx->vfe_bus_comp_grp,
			sizeof(struct cam_isp_context_comp_record) *
			CAM_IFE_BUS_COMP_NUM_MAX);
		if (c_ctx->ctx_type == CAM_IFE_CTX_TYPE_SFE && !(c_ctx->flags.is_offline))
			memcpy(query_cmd->sfe_bus_comp_grp, c_ctx->sfe_bus_comp_grp,
				sizeof(struct cam_isp_context_comp_record) *
				CAM_SFE_BUS_COMP_NUM_MAX);
		break;
	case CAM_ISP_HW_MGR_GET_LAST_CONSUMED_ADDR:
		rc = cam_ife_mgr_cmd_get_last_consumed_addr(hw_mgr_ctx,
			(struct cam_isp_hw_done_event_data *)(isp_hw_cmd_args->cmd_data));
		break;

	case CAM_ISP_HW_MGR_GET_ACTIVE_HW_CTX_CNT:
		rc = cam_ife_mgr_get_active_hw_ctx_cnt(hw_mgr_ctx, isp_hw_cmd_args);
		break;

	case CAM_ISP_HW_MGR_GET_HW_CTX:
		rc = cam_ife_mgr_get_active_hw_ctx(hw_mgr_ctx, isp_hw_cmd_args);
		break;

	default:
		CAM_ERR(CAM_ISP, "Invalid HW mgr command:0x%x, ctx_idx: %u",
			isp_hw_cmd_args->cmd_type, c_ctx->ctx_index);
		rc = -EINVAL;
		break;
	}
...
static int cam_ife_mgr_cmd(void *hw_mgr_priv, void *cmd_args)
{
	...
	case CAM_HW_MGR_CMD_INTERNAL:
		if (!hw_cmd_args->u.internal_args) {
			CAM_ERR(CAM_ISP, "Invalid cmd arguments, ctx_idx: %u", c_ctx->ctx_index);
			return -EINVAL;
		}
	...
	case CAM_HW_MGR_CMD_REG_DUMP_ON_FLUSH:
		if (c_ctx->flags.dump_on_flush)
			return 0;
		if (hw_mgr_ctx->is_offline)
			return 0;

		c_ctx->flags.dump_on_flush = true;

		rem_jiffies = cam_common_wait_for_completion_timeout(
			&c_ctx->config_done_complete, msecs_to_jiffies(30));
		if (rem_jiffies == 0)
			CAM_ERR(CAM_ISP,
	...
	case CAM_HW_MGR_CMD_REG_DUMP_ON_ERROR:
		if (c_ctx->flags.dump_on_error)
			return 0;

		if (hw_mgr_ctx->is_offline)
			return 0;

		c_ctx->flags.dump_on_error = true;
		rc = cam_ife_mgr_handle_reg_dump(hw_mgr_ctx, c_ctx->reg_dump_buf_desc,
			c_ctx->num_reg_dump_buf,
			CAM_ISP_PACKET_META_REG_DUMP_ON_ERROR, NULL, false);
		if (rc) {
			CAM_ERR(CAM_ISP,
	...
	case CAM_HW_MGR_CMD_DUMP_ACQ_INFO:
		...
		break;
	default:
		CAM_ERR(CAM_ISP, "Invalid cmd, ctx_idx: %u", c_ctx->ctx_index);
	}
		...
		struct cam_hw_dump_args *dump_args)
{
	...
	if (rc) {
		CAM_ERR(CAM_ISP,
			...
			dump_args->offset, c_ctx->ctx_index);
		goto end;
	}
...
static int cam_ife_mgr_dump(void *hw_mgr_priv, void *args)
{
	...
	if (!c_ctx) {
		...
		return -EINVAL;
	} else if (!c_ctx->num_base) {
		CAM_ERR(CAM_ISP, "ISP CTX num_base null, ctx_idx: %u", c_ctx->ctx_index);
		return -EINVAL;
	}
	...
	if (rc) {
		CAM_ERR(CAM_ISP, "Invalid handle %u rc %d ctx_idx: %u",
			dump_args->buf_handle, rc, c_ctx->ctx_index);
		return -EINVAL;
	}
	...
	if (isp_hw_dump_args.buf_len <= isp_hw_dump_args.offset) {
		CAM_ERR(CAM_ISP,
			...
			isp_hw_dump_args.offset, isp_hw_dump_args.buf_len, c_ctx->ctx_index);
		rc = -EINVAL;
		goto put_cpu_buf;
	}
	...
	for (i = 0; i < c_ctx->num_base; i++) {
		...
		switch (c_ctx->base[i].hw_type) {
		case CAM_ISP_HW_TYPE_CSID:
			hw_intf = c_ctx->hw_mgr->csid_devices[hw_idx];
			if (!hw_intf) {
			...
			break;
		case CAM_ISP_HW_TYPE_VFE:
			hw_intf = c_ctx->hw_mgr->ife_devices[hw_idx]->hw_intf;
			...
			break;
		case CAM_ISP_HW_TYPE_SFE:
			hw_intf = c_ctx->hw_mgr->sfe_devices[hw_idx]->hw_intf;
			if (!hw_intf || !hw_intf->hw_priv) {
				CAM_ERR(CAM_ISP, "hw_intf null, returning rc...ctx_idx: %u",
					c_ctx->ctx_index);
				rc = -EINVAL;
	...
	struct cam_ife_hw_event_recovery_data  *ife_mgr_recovery_data)
{
	...
	for (i = 0; i < recovery_data->no_of_context; i++) {
		c_ctx = recovery_data->affected_ctx[i];
		recovery_data->id[i] = atomic_inc_return(&c_ctx->recovery_id);
	}
	...
	uint32_t                          size)
{
	...
	bool                  rc = false;
	uint32_t              i = 0, j = 0;
	uint32_t              max_idx =  c_ctx->num_base;
	uint32_t              ctx_affected_core_idx[CAM_IFE_HW_NUM_MAX] = {0};

	CAM_DBG(CAM_ISP, "Enter:max_idx = %d, ctx_idx: %u", max_idx, c_ctx->ctx_index);

	if ((max_idx >= CAM_IFE_HW_NUM_MAX) || (size > CAM_IFE_HW_NUM_MAX)) {
		CAM_ERR_RATE_LIMIT(CAM_ISP, "invalid parameter = %d, ctx_idx: %u",
				max_idx, c_ctx->ctx_index);
		return rc;
	}

	for (i = 0; i < max_idx; i++) {
		if (affected_core[c_ctx->base[i].idx])
			rc = true;
		else {
			ctx_affected_core_idx[j] = c_ctx->base[i].idx;
			j = j + 1;
		}
	}
	...
	CAM_DBG(CAM_ISP, "Exit, ctx_idx: %u", c_ctx->ctx_index);
	return rc;
}
	...
	struct cam_ife_hw_event_recovery_data     *recovery_data)
{
	...
	list_for_each_entry(c_ctx,
		...
		atomic_set(&c_ctx->overflow_pending, 1);
		idx = c_ctx->served_ctx_id[
					c_ctx->served_ctx_w];
		if (!ife_hwr_mgr->virt_ctx_pool[idx].ctx_in_use) {
			CAM_ERR(CAM_ISP, "Invalid context");
			return -EINVAL;
		 ...
		 */
		if (notify_err_cb)
			notify_err_cb(c_ctx->common.cb_priv,
	...
	struct cam_ife_hw_mgr_ctx            *ctx)
{
	...
	cam_hw_event_cb_func ife_hw_irq_cb =
		ctx->event_cb[CAM_ISP_HW_SECONDARY_EVENT];
	...
	struct cam_isp_hw_event_info   *event_info)
{
	...
	if (!event_info->event_data) {
		CAM_ERR(CAM_ISP,
			...
			event_info->hw_idx, c_ctx->ctx_index);
		return -EINVAL;
	}
	...
	CAM_DBG(CAM_ISP, "Entry CSID[%u] error %d ctx_idx: %u",
		event_info->hw_idx, err_type, c_ctx->ctx_index);
	...
	if (event_info->is_secondary_evt) {
		...
		if (rc)
			CAM_ERR(CAM_ISP,
				...
				event_info->hw_idx, event_info->res_id, err_type, c_ctx->ctx_index);
	...
	if (recoverable && (is_bus_overflow ||
		...
		if (c_ctx->try_recovery_cnt < MAX_INTERNAL_RECOVERY_ATTEMPTS) {
			error_event_data.try_internal_recovery = true;

			if (!atomic_read(&c_ctx->overflow_pending))
				c_ctx->try_recovery_cnt++;

			if (!c_ctx->recovery_req_id)
				c_ctx->recovery_req_id = c_ctx->applied_req_id;
		}

		CAM_DBG(CAM_ISP,
			"CSID[%u] error: %u current_recovery_cnt: %u  recovery_req: %llu on ctx: %u",
			event_info->hw_idx, err_type, c_ctx->try_recovery_cnt,
			c_ctx->recovery_req_id, c_ctx->ctx_index);
	...
	struct cam_isp_hw_event_info     *event_info)
{
	...
	case CAM_IFE_PIX_PATH_RES_PPP:
		if (atomic_read(&c_ctx->overflow_pending))
			break;
		ife_hwr_irq_rup_cb(ife_ctx->cb_priv,
	...
	struct cam_isp_hw_event_info     *event_info)
{
	...
	ife_hwr_irq_rup_cb =
		ctx->event_cb[CAM_ISP_HW_EVENT_EOF];
	...
	case CAM_IFE_PIX_PATH_RES_PPP:
		if (atomic_read(&c_ctx->overflow_pending))
			break;
		ife_hwr_irq_rup_cb(ctx->cb_priv,
	...
	struct cam_isp_hw_event_info         *event_info)
{
	...
	cam_hw_event_cb_func ife_hw_irq_sof_cb
		= ctx->event_cb[CAM_ISP_HW_EVENT_SOF];
	...
	if (event_info->is_secondary_evt) {
		...
		rc = ife_hw_irq_sof_cb(ctx->cb_priv,
			CAM_ISP_HW_SECONDARY_EVENT, (void *)&sec_evt_data);
		goto end;
	}
	...
	case CAM_IFE_PIX_PATH_RES_PPP:
		...
		if (c_ctx->ctx_config &
			...
			CAM_DBG(CAM_ISP, "boot_time 0x%llx, ctx_idx: %u",
				sof_done_event_data.boot_time, c_ctx->ctx_index);
		} else {
			if (c_ctx->flags.is_offline)
	...
	struct cam_isp_hw_event_info         *event_info)
{
	...
	if (event_info->is_secondary_evt) {
		...
		rc = ife_hw_irq_epoch_cb(ctx->cb_priv,
			CAM_ISP_HW_SECONDARY_EVENT, (void *)&sec_evt_data);
		goto end;
	}
	...
	void                                 *evt_info)
{
	...
	if (event_info->res_type == CAM_ISP_RESOURCE_SFE_OUT) {
		...
		hw_mgr_res =
			&c_ctx->res_list_sfe_out[c_ctx->sfe_out_map[out_port]];
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			if (!hw_mgr_res->hw_res[i])
				continue;
			hw_intf = hw_mgr_res->hw_res[i]->hw_intf;
	...
	void                                 *evt_info)
{
	...
	if (event_info->res_type == CAM_ISP_RESOURCE_VFE_OUT) {
		out_port = event_info->res_id & 0xFF;
		hw_mgr_res =
			&c_ctx->res_list_ife_out[c_ctx->vfe_out_map[out_port]];
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			if (!hw_mgr_res->hw_res[i])
				continue;
			hw_intf = hw_mgr_res->hw_res[i]->hw_intf;
	...
	struct cam_isp_hw_event_info    *event_info)
{
	...
	CAM_DBG(CAM_ISP, "SFE[%u] error [%u] on res_type %u ctx: %u",
		...
		event_info->res_type, c_ctx->ctx_index);
	...
	if (err_evt_info->err_type & CAM_SFE_IRQ_STATUS_VIOLATION) {
		...
		CAM_DBG(CAM_ISP, "Notify context for SFE error, ctx_idx: %u", c_ctx->ctx_index);
...
static int cam_ife_hw_mgr_handle_hw_rup(
	...
	struct cam_isp_hw_event_info            *event_info)
{
	struct cam_ife_hw_mgr_ctx               *ctx = ife_hw_mgr_ctx;
	struct cam_ife_hw_concrete_ctx          *c_ctx = ctx->concr_ctx;
	cam_hw_event_cb_func                     ife_hwr_irq_rup_cb;
	struct cam_isp_hw_reg_update_event_data  rup_event_data;

	ife_hwr_irq_rup_cb =
		ctx->event_cb[CAM_ISP_HW_EVENT_REG_UPDATE];
	...
	struct cam_isp_hw_event_info         *event_info)
{
	...
	ife_hw_irq_sof_cb = ctx->event_cb[CAM_ISP_HW_EVENT_SOF];
...
{
	int rc = 0;
	struct cam_ife_hw_concrete_ctx *c_ctx = ife_hw_mgr_ctx->concr_ctx;
	...
	struct cam_isp_hw_event_info     *event_info)
{
	...
	if (!event_info->event_data) {
		CAM_ERR(CAM_ISP,
			...
			event_info->hw_type, c_ctx->ctx_index);
		return -EINVAL;
	}
...
int cam_ife_hw_mgr_event_handler(
	void                                *priv,
	uint32_t                             evt_id,
	void                                *evt_info)
{
	...
	void                                *evt_info)
{
	...
	list_for_each_entry(c_ctx, &ife_hw_mgr->used_ctx_list, list) {
		if (c_ctx->acquired_hw_id == hw_num)
			break;
	}
	...
	switch (evt_id) {
	case CAM_ISP_HW_EVENT_EPOCH:
		c_ctx->served_ctx_r = c_ctx->served_ctx_w;
		ctx_idx_rd = c_ctx->served_ctx_w;
		break;
	case CAM_ISP_HW_EVENT_SOF:
	case CAM_ISP_HW_EVENT_REG_UPDATE:
		ctx_idx_rd = c_ctx->served_ctx_w;
		break;
	case CAM_ISP_HW_EVENT_EOF:
		ctx_idx_rd = c_ctx->served_ctx_r;
		atomic_set(&c_ctx->ctx_state, CAM_IFE_HW_STATE_STARTED);
		break;
	default:
		ctx_idx_rd = c_ctx->served_ctx_r;
		break;
	...
	idx = c_ctx->served_ctx_id[ctx_idx_rd];
...
static void cam_ife_hw_mgr_dump_active_hw(char *buffer, int *offset)
{
	uint32_t i;
	struct cam_ife_hw_concrete_ctx  *c_ctx;
	struct cam_ife_hw_concrete_ctx  *c_ctx_temp;
	struct cam_isp_hw_mgr_res       *hw_mgr_res;
	struct cam_isp_hw_mgr_res       *hw_mgr_res_temp;
	...
	list_for_each_entry_safe(c_ctx, c_ctx_temp,
		&g_ife_hw_mgr.used_ctx_list, list) {
		list_for_each_entry_safe(hw_mgr_res, hw_mgr_res_temp,
			&c_ctx->res_list_ife_csid, list) {
			for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
				...
				*offset += scnprintf(buffer + *offset,
					...
					hw_mgr_res->hw_res[i]->hw_intf->hw_idx, c_ctx->ctx_index,
...
static int cam_ife_hw_mgr_debug_register(void)
{
	...
	debugfs_create_bool("disable_isp_drv", 0644, g_ife_hw_mgr.debug_cfg.dentry,
		&g_ife_hw_mgr.debug_cfg.disable_isp_drv);	debugfs_create_bool("enable_presil_reg_dump", 0644,
		g_ife_hw_mgr.debug_cfg.dentry,
	...
	void *priv_data)
{
	...
	list_for_each_entry_safe(c_ctx, c_ctx_temp,
		...
		mgr_md->ctx[i] = ctx_md;
		ctx_md->ctx_index = c_ctx->ctx_index;
		ctx_md->left_hw_idx = c_ctx->left_hw_idx;
		ctx_md->right_hw_idx = c_ctx->right_hw_idx;
		ctx_md->cdm_handle = c_ctx->cdm_handle;
		ctx_md->num_base = c_ctx->num_base;
		ctx_md->cdm_id = c_ctx->cdm_id;
		ctx_md->last_cdm_done_req = c_ctx->last_cdm_done_req;
		ctx_md->applied_req_id = c_ctx->applied_req_id;
		ctx_md->ctx_type = c_ctx->ctx_type;
		ctx_md->overflow_pending =
			atomic_read(&c_ctx->overflow_pending);
		ctx_md->cdm_done = atomic_read(&c_ctx->cdm_done);
		memcpy(&ctx_md->pf_info, &c_ctx->pf_info,
			sizeof(struct cam_ife_hw_mgr_ctx_pf_info));
		memcpy(&ctx_md->flags, &c_ctx->flags,
			sizeof(struct cam_ife_hw_mgr_ctx_flags));

		dumped_len += sizeof(*ctx_md);

		for (j = 0; j < c_ctx->num_base; j++) {
			memcpy(&ctx_md->base[j], &c_ctx->base[j],
				sizeof(struct cam_isp_ctx_base_info));
			hw_idx = c_ctx->base[j].idx;
			if (c_ctx->base[j].hw_type == CAM_ISP_HW_TYPE_CSID) {
		...
		if (c_ctx->common.mini_dump_cb) {
			hw_dump_args.start_addr = (void *)((uint8_t *)dst + dumped_len);
			hw_dump_args.len = remain_len;
			hw_dump_args.bytes_written = 0;
			rc = c_ctx->common.mini_dump_cb(c_ctx->common.cb_priv, &hw_dump_args);
...
static int cam_ife_mgr_v_acquire(void *hw_mgr_priv, void *acquire_hw_args)
{
	...
	if (!acq_args_ptr || acq_args_ptr->num_acq <= 0) {
		CAM_ERR(CAM_ISP, "Nothing to acquire. Seems like error");
		return -EINVAL;
	}
	...
	CAM_INFO(CAM_ISP, "is FE %d Offline %d ctx id %d", is_fe, is_offline, ife_mgr_ctx->ctx_idx);
	...
	if (is_offline) {
		...
		if (allocated) {
			...
			ife_mgr_ctx->concr_ctx->hw_mgr =
				acquired_hw_data->ife_ctx->hw_mgr;
...
static int cam_ife_mgr_v_stop_hw(void *hw_mgr_priv, void *stop_hw_args)
{
	...
	if (hw_mgr_ctx->is_offline) {
		...
		mutex_lock(&ife_hw_mgr->ctx_mutex);
		cam_ife_mgr_flush_in_queue(ife_hw_mgr,
			hw_mgr_ctx->ctx_idx, false, -1);

		num_ctx = atomic_read(&ife_hw_mgr->num_acquired_offline_ctx);
		if (num_ctx <=
			cam_ife_mgr_required_hw(hw_mgr_priv, true)) {
			c_ctx = NULL;
		} else {
			c_ctx =
				ife_hw_mgr->acquired_hw_pool[num_ctx-1].ife_ctx;
			if (atomic_read(&c_ctx->ctx_state) ==
...
static int cam_ife_mgr_v_recover_hw(void *priv, void *data)
{
	return cam_ife_mgr_recover_hw(priv, data);
}
	...
	uint32_t isp_device_type)
{
	...
	hw_mgr_intf->hw_recovery = cam_ife_mgr_v_recover_hw;
