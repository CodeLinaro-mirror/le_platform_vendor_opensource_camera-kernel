...
static bool cam_ife_csid_ver3_disable_sof_retime(
	...
	struct cam_isp_resource_node    *res)
{
	struct cam_ife_csid_ver3_reg_info  *csid_reg = (struct cam_ife_csid_ver3_reg_info *)
							    csid_hw->core_info->csid_reg;
	struct cam_ife_csid_ver3_path_cfg  *path_cfg = (struct cam_ife_csid_ver3_path_cfg *)
							    res->res_priv;
	const struct cam_ife_csid_ver3_path_reg_info *path_reg = csid_reg->path_reg[res->res_id];
	...
	while (debug_val) {
		...
		case CAM_IFE_CSID_DEBUG_ENABLE_SOF_IRQ:
			csid_hw->debug_info.path_mask |=
				IFE_CSID_VER3_PATH_INFO_INPUT_SOF;
			break;
		case CAM_IFE_CSID_DEBUG_ENABLE_EOF_IRQ:
			csid_hw->debug_info.path_mask |=
				IFE_CSID_VER3_PATH_INFO_INPUT_EOF;
			break;
		case CAM_IFE_CSID_DEBUG_ENABLE_SOT_IRQ:
			csid_hw->debug_info.rx_mask |=
				IFE_CSID_VER3_RX_DL0_SOT_CAPTURED |
				IFE_CSID_VER3_RX_DL1_SOT_CAPTURED |
				IFE_CSID_VER3_RX_DL2_SOT_CAPTURED;
			break;
		case CAM_IFE_CSID_DEBUG_ENABLE_EOT_IRQ:
			csid_hw->debug_info.rx_mask |=
				IFE_CSID_VER3_RX_DL0_EOT_CAPTURED |
				IFE_CSID_VER3_RX_DL1_EOT_CAPTURED |
				IFE_CSID_VER3_RX_DL2_EOT_CAPTURED;
			break;
		case CAM_IFE_CSID_DEBUG_ENABLE_SHORT_PKT_CAPTURE:
			csid_hw->debug_info.rx_mask |=
				IFE_CSID_VER3_RX_SHORT_PKT_CAPTURED;
			break;
		case CAM_IFE_CSID_DEBUG_ENABLE_LONG_PKT_CAPTURE:
			csid_hw->debug_info.rx_mask |=
				IFE_CSID_VER3_RX_LONG_PKT_CAPTURED;
			break;
		case CAM_IFE_CSID_DEBUG_ENABLE_CPHY_PKT_CAPTURE:
			csid_hw->debug_info.rx_mask |=
				IFE_CSID_VER3_RX_CPHY_PKT_HDR_CAPTURED;
			break;
		case CAM_IFE_DEBUG_ENABLE_UNMAPPED_VC_DT_IRQ:
			csid_hw->debug_info.rx_mask |=
				IFE_CSID_VER3_RX_UNMAPPED_VC_DT;
			break;
		default:
			break;
		}
...
static int cam_ife_csid_ver3_sof_irq_debug(
	struct cam_ife_csid_ver3_hw *csid_hw,
	void *cmd_args)
{
	int      i = 0;
	...
	struct   cam_ife_csid_ver3_path_cfg    *path_cfg;
	struct   cam_isp_resource_node         *res;
...
static int cam_ife_csid_ver3_get_evt_payload(
	struct cam_ife_csid_ver3_hw *csid_hw,
	struct cam_ife_csid_ver3_evt_payload **evt_payload,
...
static int cam_ife_csid_ver3_put_evt_payload(
	struct cam_ife_csid_ver3_hw *csid_hw,
	struct cam_ife_csid_ver3_evt_payload **evt_payload,
	struct list_head    *payload_list,
	spinlock_t          *lock)
...
static int cam_ife_csid_ver3_top_err_irq_top_half(
	uint32_t                                   evt_id,
	struct cam_irq_th_payload                 *th_payload)
{
	int rc = 0;
	struct cam_ife_csid_ver3_hw               *csid_hw = NULL;
	struct cam_ife_csid_ver3_evt_payload      *evt_payload;
	struct cam_ife_csid_ver3_reg_info         *csid_reg = NULL;

	csid_hw = th_payload->handler_priv;
	csid_reg = (struct cam_ife_csid_ver3_reg_info *)
			csid_hw->core_info->csid_reg;

	rc  = cam_ife_csid_ver3_get_evt_payload(csid_hw, &evt_payload,
...
static int cam_ife_csid_ver3_handle_buf_done_irq(
	uint32_t                                   evt_id,
	struct cam_irq_th_payload                 *th_payload)
{
	struct cam_ife_csid_ver3_hw               *csid_hw = NULL;
...
static int cam_ife_csid_ver3_path_err_top_half(
	uint32_t                                   evt_id,
	struct cam_irq_th_payload                 *th_payload)
{
	int32_t                                    rc;
...
static int cam_ife_csid_ver3_path_top_half(
	uint32_t                                   evt_id,
	struct cam_irq_th_payload                 *th_payload)
{
	int32_t                                    rc;
...
static inline void cam_ife_csid_ver3_reset_discard_frame_cfg(
	char                                        *res_name,
	struct cam_ife_csid_ver3_hw                 *csid_hw,
	struct cam_ife_csid_ver3_path_cfg           *path_cfg)
{
...
static int cam_ife_csid_ver3_discard_sof_top_half(
	uint32_t                                   evt_id,
	struct cam_irq_th_payload                 *th_payload)
{
	struct cam_hw_info                        *hw_info;
...
static int cam_ife_csid_ver3_discard_sof_pix_bottom_half(
	void              *handler_priv,
	void              *evt_payload_priv)
{
	...
	struct cam_isp_resource_node                 *res;
	struct cam_ife_csid_ver3_reg_info            *csid_reg = NULL;
	const struct cam_ife_csid_ver3_path_reg_info *path_reg = NULL;
	struct cam_ife_csid_ver3_path_cfg            *path_cfg;
	...
	if (path_cfg->sof_cnt == path_cfg->num_frames_discard) {
		...
		cam_ife_csid_ver3_reset_discard_frame_cfg(res->res_name, csid_hw, path_cfg);
	}
end:
...
static int cam_ife_csid_ver3_discard_sof_rdi_bottom_half(
	void              *handler_priv,
	void              *evt_payload_priv)
{
	struct cam_hw_info                           *hw_info;
...
static int cam_ife_csid_ver3_stop_csi2_in_err(
	struct cam_ife_csid_ver3_hw  *csid_hw)
{
	const struct cam_ife_csid_ver3_reg_info *csid_reg;
	struct cam_hw_soc_info                  *soc_info;
...
static int cam_ife_csid_ver3_disable_csi2(
	struct cam_ife_csid_ver3_hw  *csid_hw)
{
	const struct cam_ife_csid_ver3_reg_info *csid_reg;
	struct cam_hw_soc_info                  *soc_info;
...
static int cam_ife_csid_ver3_rx_err_top_half(
	uint32_t                                   evt_id,
	struct cam_irq_th_payload                 *th_payload)
{
	int32_t                                          rc = 0;
	...
	if (csid_hw->rx_cfg.epd_supported &&
		(th_payload->evt_status_arr[CAM_IFE_CSID_IRQ_REG_RX] ==
			IFE_CSID_VER3_RX_CPHY_EOT_RECEPTION)) {
	...
	if (status & csi2_reg->fatal_err_mask) {
		...
		cam_ife_csid_ver3_stop_csi2_in_err(csid_hw);
		goto end;
	}

	if (status & csi2_reg->part_fatal_err_mask) {
		if (status & IFE_CSID_VER3_RX_CPHY_EOT_RECEPTION)
			csid_hw->counters.error_irq_count++;

		if (status & IFE_CSID_VER3_RX_CPHY_SOT_RECEPTION)
			csid_hw->counters.error_irq_count++;

		if (status & IFE_CSID_VER3_RX_ERROR_CRC)
			csid_hw->counters.error_irq_count++;

		if (status & IFE_CSID_VER3_RX_UNBOUNDED_FRAME)
			csid_hw->counters.error_irq_count++;
...
static int cam_ife_csid_ver3_handle_rx_debug_event(
	struct cam_ife_csid_ver3_hw *csid_hw,
	uint32_t bit_pos)
{
	struct cam_hw_soc_info              *soc_info;
	...
	switch (mask) {
	case IFE_CSID_VER3_RX_LONG_PKT_CAPTURED:
	...
	case IFE_CSID_VER3_RX_SHORT_PKT_CAPTURED:
		...
		CAM_INFO_RATE_LIMIT(CAM_ISP,
			...
			csid_hw->hw_intf->hw_idx, val);
		break;
	case IFE_CSID_VER3_RX_CPHY_PKT_HDR_CAPTURED:
		...
		CAM_INFO_RATE_LIMIT(CAM_ISP,
			...
			val & csi2_reg->wc_mask);
		break;
	case IFE_CSID_VER3_RX_UNMAPPED_VC_DT:
		val = cam_io_r_mb(soc_info->reg_map[0].mem_base +
...
static int cam_ife_csid_ver3_rx_top_half(
	uint32_t                                   evt_id,
	struct cam_irq_th_payload                 *th_payload)
{
	struct cam_ife_csid_ver3_hw                *csid_hw = NULL;
	...
	while (irq_status) {
		...
		if ((BIT(bit_pos)) &
				csid_hw->debug_info.rx_mask)
			cam_ife_csid_ver3_handle_rx_debug_event(csid_hw,
				bit_pos);
		bit_pos++;
		irq_status >>= 1;
	}
...
static inline uint32_t cam_ife_csid_ver3_input_core_to_hw_idx(int core_sel)
{
	switch (core_sel) {
	case CAM_IFE_CSID_INPUT_CORE_SELECT_SFE_0: return 0;
	case CAM_IFE_CSID_INPUT_CORE_SELECT_SFE_1: return 1;
	/**
...
static int cam_ife_csid_ver3_handle_event_err(
	struct cam_ife_csid_ver3_hw  *csid_hw,
	uint32_t                      irq_status,
	uint32_t                      err_type,
	bool                          is_secondary,
	struct cam_isp_resource_node *res)
{
	...
	if (!is_secondary) {
		if (res) {
			cam_ife_csid_ver3_print_debug_reg_status(csid_hw, res);
			path_cfg = (struct cam_ife_csid_ver3_path_cfg *)res->res_priv;
	...
	evt.in_core_idx =
		cam_ife_csid_ver3_input_core_to_hw_idx(csid_hw->top_cfg.input_core_type);
...
static int cam_ife_csid_ver3_rx_err_bottom_half(
	void                                      *handler_priv,
	void                                      *evt_payload_priv)
{
	const struct cam_ife_csid_csi2_rx_reg_info *csi2_reg;
	struct cam_ife_csid_ver3_evt_payload       *payload;
	struct cam_ife_csid_ver3_hw                *csid_hw = NULL;
	struct cam_ife_csid_ver3_reg_info          *csid_reg;
	...
	if (irq_status) {
		...
		for (i = 0; i < 4; i++) {
			...
			if (irq_status & (IFE_CSID_VER3_RX_LANE0_FIFO_OVERFLOW << i)) {
		...
		if (irq_status & IFE_CSID_VER3_RX_ERROR_CPHY_PH_CRC)
			CAM_ERR_BUF(CAM_ISP, log_buf, CAM_IFE_CSID_LOG_BUF_LEN, &len,
				"CPHY_PH_CRC: Pkt Hdr CRC mismatch");

		if (irq_status & IFE_CSID_VER3_RX_STREAM_UNDERFLOW) {
			val = cam_io_r_mb(soc_info->reg_map[0].mem_base +
				csi2_reg->captured_long_pkt_0_addr);
		...
		if (irq_status & IFE_CSID_VER3_RX_ERROR_ECC)
	...
	if (irq_status) {
		...
		if (irq_status & IFE_CSID_VER3_RX_CPHY_EOT_RECEPTION)
			CAM_ERR_BUF(CAM_ISP, log_buf, CAM_IFE_CSID_LOG_BUF_LEN, &len,
		...
		if (irq_status & IFE_CSID_VER3_RX_CPHY_SOT_RECEPTION)
			CAM_ERR_BUF(CAM_ISP, log_buf, CAM_IFE_CSID_LOG_BUF_LEN, &len,
				"CPHY_SOT_RECEPTION: Less SOTs on lane/s");

		if (irq_status & IFE_CSID_VER3_RX_ERROR_CRC) {
		...
		if (irq_status & IFE_CSID_VER3_RX_UNBOUNDED_FRAME)
			CAM_ERR_BUF(CAM_ISP, log_buf, CAM_IFE_CSID_LOG_BUF_LEN, &len,
	...
	if (irq_status) {
		...
		if (irq_status & IFE_CSID_VER3_RX_MMAPPED_VC_DT) {
			val = cam_io_r_mb(soc_info->reg_map[0].mem_base +
				csi2_reg->captured_long_pkt_0_addr);
	...
	if (!csid_hw->flags.reset_awaited) {
		...
		if (event_type) {
			cam_ife_csid_ver3_handle_event_err(csid_hw,
...
void cam_ife_csid_hw_ver3_rdi_line_buffer_conflict_handler(
	void *csid)
{
	struct cam_ife_csid_ver3_hw       *csid_hw  = csid;
	struct cam_ife_csid_ver3_reg_info *csid_reg = csid_hw->core_info->csid_reg;
	struct cam_hw_soc_info            *soc_info = &csid_hw->hw_info->soc_info;
	void __iomem                      *base =
		soc_info->reg_map[CAM_IFE_CSID_CLC_MEM_BASE_ID].mem_base;
	const struct cam_ife_csid_ver3_path_reg_info *path_reg;
...
void cam_ife_csid_hw_ver3_mup_mismatch_handler(
	void *csid, void *resource)
{
	struct cam_ife_csid_ver3_hw       *csid_hw = csid;
        struct cam_isp_resource_node      *res = resource;
	struct cam_ife_csid_ver3_path_cfg *path_cfg =
		(struct cam_ife_csid_ver3_path_cfg *)res->res_priv;
...
void cam_ife_csid_ver3_print_illegal_programming_irq_status(
	void *csid, void *resource)
{
	struct cam_ife_csid_ver3_hw                        *csid_hw = csid;
	struct cam_isp_resource_node                       *res = resource;
...
static int cam_ife_csid_ver3_parse_path_irq_status(
	struct cam_ife_csid_ver3_hw  *csid_hw,
	struct cam_isp_resource_node *res,
	uint32_t                     index,
	uint32_t                     err_mask,
...
static int cam_ife_csid_ver3_top_err_irq_bottom_half(
	void                                      *handler_priv,
	void                                      *evt_payload_priv)
{
	struct cam_ife_csid_ver3_evt_payload       *payload;
...
void cam_ife_csid_ver3_print_format_measure_info(
	void *csid, void *resource)
{
	struct cam_ife_csid_ver3_hw       *csid_hw = csid;
        struct cam_isp_resource_node      *res = resource;
...
static int cam_ife_csid_ver3_ipp_bottom_half(
	void                                      *handler_priv,
	void                                      *evt_payload_priv)
{
	struct cam_ife_csid_ver3_evt_payload         *payload;
	...
	err_type = cam_ife_csid_ver3_parse_path_irq_status(
		csid_hw, res,
...
static int cam_ife_csid_ver3_ppp_bottom_half(
	void                                      *handler_priv,
	void                                      *evt_payload_priv)
{
	const struct cam_ife_csid_ver3_path_reg_info *path_reg = NULL;
...
static int cam_ife_csid_ver3_rdi_bottom_half(
	void                                      *handler_priv,
	void                                      *evt_payload_priv)
{
	struct cam_ife_csid_ver3_evt_payload         *payload;
	...
	struct cam_ife_csid_ver3_path_cfg            *path_cfg;
	struct cam_isp_resource_node                 *res;
	const struct cam_ife_csid_ver3_path_reg_info *rdi_reg;
	struct cam_hw_info                           *hw_info;
	...
	err_type = cam_ife_csid_ver3_parse_path_irq_status(csid_hw, res,
	...
	if (path_cfg->sec_evt_config.en_secondary_evt &&
		((irq_status_rdi & IFE_CSID_VER3_PATH_SENSOR_SWITCH_OUT_OF_SYNC_FRAME_DROP) &&
		(path_cfg->sec_evt_config.evt_type & CAM_IFE_CSID_EVT_SENSOR_SYNC_FRAME_DROP)))
		cam_ife_csid_ver3_handle_event_err(csid_hw, irq_status_rdi,
...
int cam_ife_csid_ver3_get_hw_caps(void *hw_priv,
...
static int cam_ife_csid_ver3_wait_for_reset(
	struct cam_ife_csid_ver3_hw *csid_hw)
{
...
static int cam_ife_csid_ver3_reset_irq_top_half(uint32_t    evt_id,
	struct cam_irq_th_payload         *th_payload)
{
	struct cam_ife_csid_ver3_hw *csid_hw;
...
static int cam_ife_csid_ver3_internal_reset(
	struct cam_ife_csid_ver3_hw *csid_hw,
	uint32_t rst_cmd, uint32_t rst_location, uint32_t rst_mode)
{
	...
	if (csid_hw->sync_mode == CAM_ISP_HW_SYNC_SLAVE &&
		rst_cmd == CAM_IFE_CSID_RESET_COMMAND_HW_RST)
		goto wait_only;

	if (rst_cmd == CAM_IFE_CSID_RESET_COMMAND_SW_RST)
		reinit_completion(&csid_hw->hw_info->hw_complete);

	/*Program the reset location */
	if (rst_location == CAM_IFE_CSID_RESET_LOCATION_PATH_ONLY)
		val |= (csid_reg->cmn_reg->rst_loc_path_only_val <<
		       csid_reg->cmn_reg->rst_location_shift_val);
	else if (rst_location == CAM_IFE_CSID_RESET_LOCATION_COMPLETE)
	...
	/*Program the cmd */
	if (rst_cmd == CAM_IFE_CSID_RESET_COMMAND_IRQ_CTRL)
		val = csid_reg->cmn_reg->rst_cmd_irq_ctrl_only_val;
	else if (rst_cmd == CAM_IFE_CSID_RESET_COMMAND_HW_RST)
		val = csid_reg->cmn_reg->rst_cmd_hw_reset_complete_val;
	else if (rst_cmd == CAM_IFE_CSID_RESET_COMMAND_SW_RST)
...
wait_only:
	...
	rc = cam_ife_csid_ver3_wait_for_reset(csid_hw);
...
int cam_ife_csid_ver3_reset(void *hw_priv,
	void *reset_args, uint32_t arg_size)
{
	...
	case CAM_IFE_CSID_RESET_GLOBAL:
		rc = cam_ife_csid_ver3_internal_reset(csid_hw,
			CAM_IFE_CSID_RESET_COMMAND_SW_RST,
	...
	case CAM_IFE_CSID_RESET_PATH:
		rc = cam_ife_csid_ver3_internal_reset(csid_hw,
			...
			CAM_IFE_CSID_RESET_LOCATION_PATH_ONLY,
			CAM_CSID_HALT_IMMEDIATELY);
		break;
...
static int cam_ife_csid_ver3_disable_path(
	struct cam_ife_csid_ver3_hw     *csid_hw,
	struct cam_isp_resource_node    *res)
{
	struct cam_ife_csid_ver3_path_cfg       *path_cfg;
...
static int cam_ife_csid_ver3_decode_format1_validate(
	struct cam_ife_csid_ver3_hw *csid_hw,
	struct cam_isp_resource_node    *res)
{
	int rc = 0;
	const struct cam_ife_csid_ver3_reg_info *csid_reg =
		(struct cam_ife_csid_ver3_reg_info *)csid_hw->core_info->csid_reg;
	struct cam_ife_csid_ver3_path_cfg *path_cfg =
		(struct cam_ife_csid_ver3_path_cfg *)res->res_priv;
	struct cam_ife_csid_cid_data *cid_data = &csid_hw->cid_data[path_cfg->cid];
...
static bool cam_ife_csid_hw_ver3_need_unpack_mipi(
	struct cam_ife_csid_ver3_hw                  *csid_hw,
...
static int cam_ife_csid_hw_ver3_config_path_data(
	struct cam_ife_csid_ver3_hw *csid_hw,
	struct cam_ife_csid_ver3_path_cfg *path_cfg,
	struct cam_csid_hw_reserve_resource_args  *reserve,
	uint32_t cid)
{
	...
	if (!reserve->per_port_acquire) {
		...
		case CAM_IFE_PIX_PATH_RES_RDI_4:
			 ...
			 * taken at WM side
			 */
			reserve->use_wm_pack = cam_ife_csid_hw_ver3_need_unpack_mipi(csid_hw,
		...
		if (csid_reg->cmn_reg->decode_format1_supported &&
			(cid_data->vc_dt[CAM_IFE_CSID_MULTI_VC_DT_GRP_1].valid)) {
			rc = cam_ife_csid_ver3_decode_format1_validate(csid_hw, res);
			if (rc) {
...
static int cam_ife_csid_hw_ver3_config_rx(
	struct cam_ife_csid_ver3_hw *csid_hw,
	struct cam_csid_hw_reserve_resource_args  *reserve)
{
...
int cam_ife_csid_hw_ver3_hw_cfg(
	struct cam_ife_csid_ver3_hw *csid_hw,
	struct cam_ife_csid_ver3_path_cfg *path_cfg,
	struct cam_csid_hw_reserve_resource_args  *reserve,
	uint32_t cid)
{
	...
	rc = cam_ife_csid_hw_ver3_config_rx(csid_hw, reserve);
	...
	cam_ife_csid_hw_ver3_config_path_data(csid_hw, path_cfg,
		reserve, cid);
...
bool cam_ife_csid_ver3_is_width_valid(
	struct cam_csid_hw_reserve_resource_args  *reserve,
	struct cam_ife_csid_ver3_hw *csid_hw)
{
	struct cam_csid_soc_private *soc_private = NULL;
...
static int cam_ife_csid_ver3_in_port_validate(
	struct cam_csid_hw_reserve_resource_args  *reserve,
	struct cam_ife_csid_ver3_hw     *csid_hw,
	bool is_per_port_acquire)
{
	...
	if (!cam_ife_csid_ver3_is_width_valid(reserve, csid_hw))
		goto err;
...
int cam_ife_csid_ver3_reserve(void *hw_priv,
	void *reserve_args, uint32_t arg_size)
{
	...
	rc = cam_ife_csid_ver3_in_port_validate(reserve, csid_hw, is_per_port_acquire);
	if (rc) {
	...
	if (!reserve->is_offline) {
		rc = cam_ife_csid_hw_ver3_hw_cfg(csid_hw, path_cfg,
			reserve, cid);
		if (rc) {
...
int cam_ife_csid_ver3_release(void *hw_priv,
	void *release_args, uint32_t arg_size)
{
	...
	if (!csid_hw->counters.csi2_reserve_cnt) {
		...
		memset(&csid_hw->top_cfg, 0,
			sizeof(struct cam_ife_csid_ver3_top_cfg));
		memset(&csid_hw->debug_info, 0,
			sizeof(struct cam_ife_csid_debug_info));
...
static int cam_ife_csid_ver3_res_master_slave_cfg(
	struct cam_ife_csid_ver3_hw *csid_hw,
	uint32_t res_id)
{
	struct cam_ife_csid_ver3_reg_info *csid_reg;
    ...
    val = cam_io_r_mb(mem_base +
               csid_reg->cmn_reg->group_pri_sec_cfg_addr);
...
static int cam_ife_csid_ver3_init_config_rdi_path(
	struct cam_ife_csid_ver3_hw *csid_hw,
	struct cam_isp_resource_node    *res)
{
	int rc = 0;
	...
	struct cam_hw_soc_info                   *soc_info;
	const struct cam_ife_csid_ver3_path_reg_info *path_reg = NULL;
	const struct cam_ife_csid_ver3_common_reg_info *cmn_reg = NULL;
	...
	/* configure domain id */
	if (path_reg->domain_id_supported)
	{
		val = 0;
		val = 0 << path_reg->domain_id_shift_val;
		if (path_reg->domain_id_cfg_num == 0)
			cam_io_w_mb(val, mem_base + cmn_reg->domain_id_cfg0_addr);
		else if (path_reg->domain_id_cfg_num == 1)
			cam_io_w_mb(val, mem_base + cmn_reg->domain_id_cfg1_addr);
	}
	/* Enable the RDI path */
	...
	if (path_cfg->sfe_shdr ||
		...
		cam_ife_csid_ver3_res_master_slave_cfg(csid_hw, res->res_id);
...
static int cam_ife_csid_ver3_init_config_pxl_path(
	struct cam_ife_csid_ver3_hw *csid_hw,
	struct cam_isp_resource_node    *res)
{
	int rc = 0;
...
static inline int cam_ife_csid_ver3_subscribe_sof_for_discard(
	struct cam_ife_csid_ver3_path_cfg *path_cfg,
	struct cam_ife_csid_ver3_hw       *csid_hw,
	struct cam_isp_resource_node      *res,
	...
	CAM_IRQ_HANDLER_BOTTOM_HALF        bottom_half_handler)
{
	...
	val = IFE_CSID_VER3_PATH_INFO_INPUT_SOF;
	path_cfg->stored_irq_masks[CAM_IFE_CSID_SOF_DISCARD_MSK][path_cfg->irq_reg_idx] = val;
	path_cfg->discard_irq_handle = cam_irq_controller_subscribe_irq(
		csid_hw->csid_irq_controller,
		CAM_IRQ_PRIORITY_0,
		path_cfg->stored_irq_masks[CAM_IFE_CSID_SOF_DISCARD_MSK],
		res,
		top_half_handler,
		bottom_half_handler,
		res->workq_info,
...
static int cam_ife_csid_ver3_program_rdi_path(
	struct cam_ife_csid_ver3_hw     *csid_hw,
	struct cam_isp_resource_node    *res,
	uint32_t                        *rup_aup_mask)
{
	...
	rc = cam_ife_csid_ver3_init_config_rdi_path(
		csid_hw, res);
	...
	path_cfg->stored_irq_masks[CAM_IFE_CSID_TOP_MSK][CAM_IFE_CSID_IRQ_REG_TOP] =
		path_reg->top_irq_mask;

	path_cfg->stored_irq_masks[CAM_IFE_CSID_TOP_MSK][path_cfg->irq_reg_idx] = val;

	/*Get mask for err irq*/
	val = path_reg->fatal_err_mask | path_reg->non_fatal_err_mask;
	path_cfg->stored_irq_masks[CAM_IFE_CSID_ERR_MSK][CAM_IFE_CSID_IRQ_REG_TOP] =
		path_reg->top_irq_mask;
	path_cfg->stored_irq_masks[CAM_IFE_CSID_ERR_MSK][path_cfg->irq_reg_idx] = val;
	...
	path_cfg->irq_handle = cam_irq_controller_subscribe_irq(
		csid_hw->csid_irq_controller,
		CAM_IRQ_PRIORITY_1,
		path_cfg->stored_irq_masks[CAM_IFE_CSID_TOP_MSK],
		res,
		cam_ife_csid_ver3_path_top_half,
		cam_ife_csid_ver3_rdi_bottom_half,
		res->workq_info,
		&workq_bh_api,
		CAM_IRQ_EVT_GROUP_0);
	...
	if (path_cfg->discard_init_frames) {
		rc = cam_ife_csid_ver3_subscribe_sof_for_discard(
			path_cfg, csid_hw, res,
			cam_ife_csid_ver3_discard_sof_top_half,
			cam_ife_csid_ver3_discard_sof_rdi_bottom_half);
		if (rc)
			goto end;
	}
	...
	path_cfg->err_irq_handle = cam_irq_controller_subscribe_irq(
			csid_hw->csid_irq_controller,
			CAM_IRQ_PRIORITY_0,
			path_cfg->stored_irq_masks[CAM_IFE_CSID_ERR_MSK],
			res,
			cam_ife_csid_ver3_path_err_top_half,
			cam_ife_csid_ver3_rdi_bottom_half,
			res->workq_info,
			&workq_bh_api,
			CAM_IRQ_EVT_GROUP_0);
...
static int cam_ife_csid_ver3_program_ipp_path(
	struct cam_ife_csid_ver3_hw    *csid_hw,
	struct cam_isp_resource_node   *res,
	uint32_t                       *rup_aup_mask)
{
	...
	rc = cam_ife_csid_ver3_init_config_pxl_path(
		csid_hw, res);
	...
	path_cfg->irq_handle = cam_irq_controller_subscribe_irq(
				    ...
				    res,
				    cam_ife_csid_ver3_path_top_half,
				    cam_ife_csid_ver3_ipp_bottom_half,
				    res->workq_info,
				    &workq_bh_api,
				    CAM_IRQ_EVT_GROUP_0);
	...
	if (path_cfg->discard_init_frames) {
		rc = cam_ife_csid_ver3_subscribe_sof_for_discard(
			path_cfg, csid_hw, res,
			cam_ife_csid_ver3_discard_sof_top_half,
			cam_ife_csid_ver3_discard_sof_pix_bottom_half);
		if (rc)
			goto end;
	}
...
static int cam_ife_csid_ver3_enable_path(
	struct cam_ife_csid_ver3_hw *csid_hw,
	struct cam_isp_resource_node    *res)
{
	const struct cam_ife_csid_ver3_reg_info      *csid_reg;
...
static int cam_ife_csid_ver3_program_ppp_path(
	struct cam_ife_csid_ver3_hw     *csid_hw,
	struct cam_isp_resource_node    *res,
	uint32_t                        *rup_aup_mask)
{
	...
	path_cfg->irq_handle = cam_irq_controller_subscribe_irq(
				...
				csid_hw,
				cam_ife_csid_ver3_path_top_half,
				cam_ife_csid_ver3_ppp_bottom_half,
				res->workq_info,
				&workq_bh_api,
				CAM_IRQ_EVT_GROUP_0);
...
static int cam_ife_csid_ver3_rx_capture_config(
	struct cam_ife_csid_ver3_hw *csid_hw)
{
	const struct cam_ife_csid_ver3_reg_info   *csid_reg;
	struct cam_hw_soc_info                    *soc_info;
	...
	csid_reg = (struct cam_ife_csid_ver3_reg_info *) csid_hw->core_info->csid_reg;
	soc_info = &csid_hw->hw_info->soc_info;
...
static int cam_ife_csid_ver3_enable_csi2(struct cam_ife_csid_ver3_hw *csid_hw)
{
	int rc = 0;
	struct cam_hw_soc_info              *soc_info;
	const struct cam_ife_csid_ver3_reg_info *csid_reg;
	...
	if (csid_hw->rx_cfg.epd_supported &&
		...
		val &= ~IFE_CSID_VER3_RX_CPHY_EOT_RECEPTION;

	if (csid_hw->debug_info.debug_val & CAM_IFE_CSID_DEBUG_DISABLE_CRC)
		val &= ~IFE_CSID_VER3_RX_ERROR_CRC;
	...
	csid_hw->rx_cfg.err_irq_handle =
			    cam_irq_controller_subscribe_irq(
				    ...
				    irq_mask,
				    csid_hw,
				    cam_ife_csid_ver3_rx_err_top_half,
				    cam_ife_csid_ver3_rx_err_bottom_half,
				    csid_hw->workq,
				    &workq_bh_api,
				    CAM_IRQ_EVT_GROUP_0);
	...
	cam_ife_csid_ver3_rx_capture_config(csid_hw);
...
static int cam_ife_csid_ver3_program_top(
	struct cam_ife_csid_ver3_hw *csid_hw)
{
	const struct cam_ife_csid_ver3_top_reg_info *top_reg;
	struct cam_ife_csid_ver3_reg_info *csid_reg;
	uint32_t val;
	struct cam_hw_soc_info              *soc_info;
	int input_core_sel;
...
static int cam_ife_csid_ver3_enable_core(struct cam_ife_csid_ver3_hw *csid_hw)
{
	int rc = 0;
	struct cam_hw_soc_info              *soc_info;
	const struct cam_ife_csid_ver3_reg_info *csid_reg;
	...
	csid_hw->reset_irq_handle = cam_irq_controller_subscribe_irq(
		...
		irq_mask,
		csid_hw,
		cam_ife_csid_ver3_reset_irq_top_half,
		NULL,
		NULL,
		NULL,
		CAM_IRQ_EVT_GROUP_0);
	...
	reinit_completion(&csid_hw->hw_info->hw_complete);
	cam_ife_csid_ver3_program_top(csid_hw);
...
static int cam_ife_csid_ver3_enable_hw(
	struct cam_ife_csid_ver3_hw *csid_hw)
{
	struct cam_hw_soc_info              *soc_info;
	const struct cam_ife_csid_ver3_reg_info *csid_reg = NULL;
	uint32_t  val;
	int i, rc;
	void __iomem *mem_base;
	...
	csid_hw->buf_done_irq_handle = cam_irq_controller_subscribe_irq(
		...
		buf_done_irq_mask,
		csid_hw,
		cam_ife_csid_ver3_handle_buf_done_irq,
		NULL,
		NULL,
		NULL,
		CAM_IRQ_EVT_GROUP_0);
	...
	csid_hw->top_err_irq_handle = cam_irq_controller_subscribe_irq(
		...
		top_err_irq_mask,
		csid_hw,
		cam_ife_csid_ver3_top_err_irq_top_half,
		cam_ife_csid_ver3_top_err_irq_bottom_half,
		csid_hw->workq,
		&workq_bh_api,
		CAM_IRQ_EVT_GROUP_0);
...
int cam_ife_csid_ver3_init_hw(void *hw_priv,
	void *init_args, uint32_t arg_size)
{
	...
	rc = cam_ife_csid_ver3_enable_core(csid_hw);
...
static int cam_ife_csid_ver3_disable_core(
	struct cam_ife_csid_ver3_hw *csid_hw)
{
	const struct cam_ife_csid_ver3_reg_info *csid_reg;
	const struct cam_ife_csid_ver3_top_reg_info *top_reg = NULL;
	struct cam_hw_soc_info                   *soc_info;
	struct cam_csid_soc_private              *soc_private;
	int rc = 0;
...
int cam_ife_csid_ver3_deinit_hw(void *hw_priv,
...
disable_hw:
	res->res_state = CAM_ISP_RESOURCE_STATE_RESERVED;
	cam_ife_csid_ver3_disable_core(csid_hw);
	mutex_unlock(&csid_hw->hw_info->hw_mutex);
...
int cam_ife_csid_ver3_start(void *hw_priv, void *args,
			uint32_t arg_size)
{
	struct cam_ife_csid_ver3_hw           *csid_hw  = NULL;
	struct cam_isp_resource_node          *res;
...
int cam_ife_csid_ver3_stop(void *hw_priv,
	void *stop_args, uint32_t arg_size)
{
	...
	cam_ife_csid_ver3_reset(hw_priv, &reset,
...
int cam_ife_csid_ver3_read(void *hw_priv,
...
int cam_ife_csid_ver3_write(void *hw_priv,
...
static int cam_ife_csid_ver3_top_cfg(
	struct cam_ife_csid_ver3_hw *csid_hw, void *cmd_args)
{
	...
	case CAM_IFE_CSID_INPUT_CORE_NONE:
		csid_hw->top_cfg.input_core_type =
			CAM_IFE_CSID_INPUT_CORE_SELECT_NONE;
		csid_hw->top_cfg.out_ife_en = false;
		break;

	case CAM_IFE_CSID_INPUT_CORE_IFE:
		csid_hw->top_cfg.input_core_type =
			CAM_IFE_CSID_INPUT_CORE_SELECT_INTERNAL;
		break;
	...
	case CAM_IFE_CSID_INPUT_CORE_SFE_IFE:
		...
		if (top_args->core_idx == 0) {
			csid_hw->top_cfg.input_core_type =
				CAM_IFE_CSID_INPUT_CORE_SELECT_SFE_0;
		} else if (top_args->core_idx == 1) {
			csid_hw->top_cfg.input_core_type =
				CAM_IFE_CSID_INPUT_CORE_SELECT_SFE_1;
		} else {
			rc = -EINVAL;
			CAM_ERR(CAM_ISP,
...
static int cam_ife_csid_ver3_reg_update(
	struct cam_ife_csid_ver3_hw   *csid_hw,
...
static int cam_ife_csid_ver3_program_offline_go_cmd(
	struct cam_ife_csid_ver3_hw   *csid_hw,
	void *cmd_args, uint32_t arg_size)
{
...
static uint64_t __cam_ife_csid_ver3_get_time_stamp(void __iomem *mem_base, uint32_t timestamp0_addr,
...
static int cam_ife_csid_ver3_get_time_stamp(
	struct cam_ife_csid_ver3_hw  *csid_hw, void *cmd_args)
{
	const struct cam_ife_csid_ver3_path_reg_info *path_reg;
	struct cam_isp_resource_node         *res = NULL;
	...
	if (timestamp_args->get_prev_timestamp) {
		timestamp_args->prev_time_stamp_val = __cam_ife_csid_ver3_get_time_stamp(
			soc_info->reg_map[0].mem_base,
			path_reg->timestamp_perv0_sof_addr,
			path_reg->timestamp_perv1_sof_addr);
	}

	timestamp_args->time_stamp_val = __cam_ife_csid_ver3_get_time_stamp(
		soc_info->reg_map[0].mem_base,
...
static int cam_ife_csid_ver3_print_hbi_vbi(
	struct cam_ife_csid_ver3_hw  *csid_hw,
	struct cam_isp_resource_node *res)
{
	struct cam_hw_soc_info              *soc_info;
...
static int cam_ife_csid_ver3_set_dynamic_switch_config(
	struct cam_ife_csid_ver3_hw *csid_hw,
	void                        *cmd_args)
{
...
static int cam_ife_csid_ver3_set_csid_clock(
	struct cam_ife_csid_ver3_hw          *csid_hw,
	void *cmd_args)
{
...
static int cam_ife_csid_ver3_mini_dump(
	struct cam_ife_csid_ver3_hw  *csid_hw,
	void *cmd_args)
{
	struct cam_ife_csid_ver3_mini_dump_data *md;
	uint32_t                                 i  = 0;
	struct cam_ife_csid_ver3_path_cfg       *path_cfg;
	struct cam_ife_csid_ver3_res_mini_dump  *md_res;
	struct cam_isp_resource_node            *res;
	...
	md  = (struct cam_ife_csid_ver3_mini_dump_data *)
...
static int cam_ife_csid_ver3_dual_sync_cfg(
	struct cam_ife_csid_ver3_hw  *csid_hw,
	void *cmd_args)
{
...
static int cam_ife_csid_ver3_set_discard_frame_cfg(
	struct cam_ife_csid_ver3_hw    *csid_hw,
	void                           *cmd_args)
{
	struct cam_isp_resource_node                 *res;
...
static int cam_ife_csid_ver3_rdi_lcr_cfg(
	struct cam_ife_csid_ver3_hw  *csid_hw, void *cmd_args)
{
	const struct cam_ife_csid_ver3_path_reg_info *path_reg;
	struct cam_ife_csid_ver3_reg_info            *csid_reg;
	struct cam_ife_csid_ver3_path_cfg            *path_cfg = NULL;
...
static int cam_ife_csid_ver3_update_res_data(struct cam_ife_csid_ver3_hw *csid_hw,
	void *reserve_args)
{
	...
	rc = cam_ife_csid_ver3_in_port_validate(reserve, csid_hw, res->is_per_port_acquire);
	if (rc) {
		CAM_ERR(CAM_ISP, "CSID %d Res_id %d port validation failed",
			csid_hw->hw_intf->hw_idx, reserve->res_id);
		return rc;
...
static int cam_ife_csid_ver3_subscribe_path_irqs(
	struct cam_ife_csid_ver3_hw     *csid_hw,
	struct cam_isp_resource_node    *res,
...
static int cam_ife_csid_ver3_update_path_irq(
	struct cam_ife_csid_ver3_hw     *csid_hw,
	struct cam_isp_resource_node    *res,
	bool                             enable)
{
	...
	if (!res->is_per_port_acquire && !path_cfg->irq_handle && !res->is_per_port_start) {
		switch (res->res_id) {
		case  CAM_IFE_PIX_PATH_RES_IPP:
			bh_handler = cam_ife_csid_ver3_ipp_bottom_half;
			sof_discard_bh = cam_ife_csid_ver3_discard_sof_pix_bottom_half;
			rc = cam_ife_csid_ver3_subscribe_path_irqs(csid_hw,
				res,
				bh_handler,
				sof_discard_bh);
			break;
		case  CAM_IFE_PIX_PATH_RES_PPP:
			bh_handler = cam_ife_csid_ver3_ppp_bottom_half;
			sof_discard_bh = cam_ife_csid_ver3_discard_sof_pix_bottom_half;
			rc = cam_ife_csid_ver3_subscribe_path_irqs(csid_hw,
				res,
				bh_handler,
		...
		case CAM_IFE_PIX_PATH_RES_RDI_3:
		case CAM_IFE_PIX_PATH_RES_RDI_4:
			bh_handler = cam_ife_csid_ver3_rdi_bottom_half;
			sof_discard_bh = cam_ife_csid_ver3_discard_sof_rdi_bottom_half;
			rc = cam_ife_csid_ver3_subscribe_path_irqs(csid_hw,
				res,
				bh_handler,
				sof_discard_bh);
...
static int cam_ife_csid_ver3_update_irq_mask(
	struct cam_ife_csid_ver3_hw *csid_hw,
	void *res_irq_mask)
{
	struct cam_isp_resource_node          *res;
	...
	for (i = 0; i < irq_args->num_res; i++) {
		...
		rc = cam_ife_csid_ver3_update_path_irq(csid_hw, res, irq_args->enable_irq);
		if (rc)
			goto end;
	}
end:
...
static int cam_ife_csid_ver3_process_cmd(void *hw_priv,
	uint32_t cmd_type, void *cmd_args, uint32_t arg_size)
{
	...
	case CAM_IFE_CSID_CMD_GET_TIME_STAMP:
		rc = cam_ife_csid_ver3_get_time_stamp(csid_hw, cmd_args);

		if (csid_hw->debug_info.debug_val &
				CAM_IFE_CSID_DEBUG_ENABLE_HBI_VBI_INFO) {
			res = ((struct cam_csid_get_time_stamp_args *)
				cmd_args)->node_res;
			cam_ife_csid_ver3_print_hbi_vbi(csid_hw, res);
		}
		break;
	case CAM_IFE_CSID_SET_CSID_DEBUG:
		rc = cam_ife_csid_ver3_set_debug(csid_hw,
			*((uint32_t *)cmd_args));
		break;
	case CAM_IFE_CSID_SOF_IRQ_DEBUG:
		rc = cam_ife_csid_ver3_sof_irq_debug(csid_hw, cmd_args);
		break;
	case CAM_ISP_HW_CMD_CSID_CLOCK_UPDATE:
		rc = cam_ife_csid_ver3_set_csid_clock(csid_hw, cmd_args);
		break;
	case CAM_ISP_HW_CMD_DUMP_HW:
		break;
	case CAM_IFE_CSID_TOP_CONFIG:
		rc = cam_ife_csid_ver3_top_cfg(csid_hw, cmd_args);
		break;
	...
	case CAM_ISP_HW_CMD_GET_REG_UPDATE:
		rc = cam_ife_csid_ver3_reg_update(csid_hw,
			cmd_args, arg_size);
		break;
	case CAM_IFE_CSID_SET_DUAL_SYNC_CONFIG:
		rc = cam_ife_csid_ver3_dual_sync_cfg(csid_hw,
			cmd_args);
		break;
	case CAM_ISP_HW_CSID_MINI_DUMP:
		rc  = cam_ife_csid_ver3_mini_dump(csid_hw, cmd_args);
		break;
	case CAM_IFE_CSID_PROGRAM_OFFLINE_CMD:
		rc = cam_ife_csid_ver3_program_offline_go_cmd(
			csid_hw, cmd_args, arg_size);
		break;
	case CAM_ISP_HW_CMD_CSID_DYNAMIC_SWITCH_UPDATE:
		rc = cam_ife_csid_ver3_set_dynamic_switch_config(csid_hw, cmd_args);
		break;
	...
	case CAM_ISP_HW_CMD_CSID_DISCARD_INIT_FRAMES:
		rc = cam_ife_csid_ver3_set_discard_frame_cfg(csid_hw, cmd_args);
		break;
	case CAM_ISP_HW_CMD_RDI_LCR_CFG:
		rc = cam_ife_csid_ver3_rdi_lcr_cfg(csid_hw, cmd_args);
		break;
	...
	case CAM_ISP_HW_CMD_UPDATE_CSID_RES_DATA:
		rc = cam_ife_csid_ver3_update_res_data(csid_hw, cmd_args);
		break;
	case CAM_ISP_HW_CMD_UPDATE_CSID_RES_IRQ_MASK:
		rc = cam_ife_csid_ver3_update_irq_mask(csid_hw, cmd_args);
		break;
	default:
...
{
	struct cam_ife_csid_ver3_hw *csid_hw = data;
...
static void cam_ife_csid_ver3_free_res(struct cam_ife_csid_ver3_hw *csid_hw)
{
...
static int cam_ife_ver3_hw_alloc_res(
	struct cam_isp_resource_node *res,
	uint32_t res_type,
...
static int cam_ife_csid_ver3_hw_init_path_res(
	struct cam_ife_csid_ver3_hw   *csid_hw)
{
	int rc = 0;
	int i;
...
free_res:
	cam_ife_csid_ver3_free_res(csid_hw);
	return rc;
}
...
int cam_ife_csid_hw_ver3_init(struct cam_hw_intf *hw_intf,
	...
	bool is_custom)
{
	...
	csid_hw = kzalloc(sizeof(struct cam_ife_csid_ver3_hw), GFP_KERNEL);
	...
	csid_hw->hw_intf->hw_ops.get_hw_caps =
						cam_ife_csid_ver3_get_hw_caps;
	csid_hw->hw_intf->hw_ops.init        = cam_ife_csid_ver3_init_hw;
	csid_hw->hw_intf->hw_ops.deinit      = cam_ife_csid_ver3_deinit_hw;
	csid_hw->hw_intf->hw_ops.reset       = cam_ife_csid_ver3_reset;
	csid_hw->hw_intf->hw_ops.reserve     = cam_ife_csid_ver3_reserve;
	csid_hw->hw_intf->hw_ops.release     = cam_ife_csid_ver3_release;
	csid_hw->hw_intf->hw_ops.start       = cam_ife_csid_ver3_start;
	csid_hw->hw_intf->hw_ops.stop        = cam_ife_csid_ver3_stop;
	csid_hw->hw_intf->hw_ops.read        = cam_ife_csid_ver3_read;
	csid_hw->hw_intf->hw_ops.write       = cam_ife_csid_ver3_write;
	csid_hw->hw_intf->hw_ops.process_cmd =
						cam_ife_csid_ver3_process_cmd;

	rc = cam_ife_csid_hw_init_irq(csid_hw);

	rc = cam_ife_csid_ver3_hw_init_path_res(csid_hw);
...
EXPORT_SYMBOL(cam_ife_csid_hw_ver3_init);

int cam_ife_csid_hw_ver3_deinit(struct cam_hw_info *hw_priv)
{
	struct cam_ife_csid_ver3_hw   *csid_hw;
	unsigned long flags;
	int rc = -EINVAL;
	...
	csid_hw = (struct cam_ife_csid_ver3_hw *)hw_priv->core_info;
...
EXPORT_SYMBOL(cam_ife_csid_hw_ver3_deinit);
