...
static int cam_icp_update_qos(struct cam_icp_hw_mgr *hw_mgr)
{
	...
	set_prop->num_prop = 1;
	set_prop->prop_data_flex[0] = HFI_PROPERTY_QOS_PARAMS;

	qos_data = (struct hfi_cmd_qos_params *)&set_prop->prop_data_flex[1];
	...
	for (i = 0; i < CAM_ICP_CTX_MAX; i++) {
		...
		if (ctx_data->state == CAM_ICP_CTX_STATE_ACQUIRED &&
			...
			fmin = ((ctx_data->clk_info.curr_fc * mul) /
					ctx_data->clk_info.budget_ns);
			fps = mul / ctx_data->clk_info.budget_ns;
			qos_data->qos_params_flex[num_ent].target_completion_cycles =
				(fmin * core_freq) /
					(total_fmin * fps);
			qos_data->qos_params_flex[num_ent].session_id = ctx_data->fw_handle;
			CAM_DBG(CAM_ICP, "ctx %d core freq %llu tgc %llu fmin %llu",
				ctx_data->ctx_id, core_freq,
				qos_data->qos_params_flex[num_ent].target_completion_cycles, fmin);
			num_ent++;
		}
	}
	CAM_DBG(CAM_ICP,
...
static void cam_icp_free_hfi_mem(void)
{
	...
	if (icp_hw_mgr.fw_based_sys_caching)
...
static int cam_icp_allocate_llcc_register_mem(struct cam_icp_hw_mgr *hw_mgr)
{
	...
	if (rc) {
		CAM_ERR(CAM_ICP, "Failed in alloc llcc mem rc %d", rc);
		return rc;
	}
...
static int cam_icp_allocate_device_synx_hwmutex_mem(void)
{
	int rc;
	size_t len;
	dma_addr_t iova;
	...
	icp_hw_mgr.hfi_mem.synx_hwmutex.len = len;
	icp_hw_mgr.hfi_mem.synx_hwmutex.iova = iova;
	icp_hw_mgr.hfi_mem.synx_hwmutex.smmu_hdl = icp_hw_mgr.iommu_hdl;
...
static int cam_icp_allocate_device_global_cnt_mem(void)
{
	int rc;
	size_t len;
	dma_addr_t iova;
	...
	icp_hw_mgr.hfi_mem.global_cntr.len = len;
	icp_hw_mgr.hfi_mem.global_cntr.iova = iova;
	icp_hw_mgr.hfi_mem.global_cntr.smmu_hdl = icp_hw_mgr.iommu_hdl;
...
static int cam_icp_allocate_device_soc_version_mem(void)
{
	int rc;
	size_t len;
	dma_addr_t iova;
	...
	rc = cam_smmu_map_phy_mem_region(icp_hw_mgr.iommu_hdl,
		CAM_SMMU_REGION_DEVICE, CAM_SMMU_SUBREGION_SOC_HW_VERSION,
		&iova, &len);
	if (rc) {
		CAM_ERR(CAM_ICP,
			"Failed in allocating soc hw version mem rc %d", rc);
		return rc;
	}

	icp_hw_mgr.hfi_mem.soc_hw_version.len = len;
	icp_hw_mgr.hfi_mem.soc_hw_version.iova = iova;
	icp_hw_mgr.hfi_mem.soc_hw_version.smmu_hdl = icp_hw_mgr.iommu_hdl;
...
static int cam_icp_allocate_device_ipc_hwmutex_mem(void)
{
	int rc;
	size_t len;
	dma_addr_t iova;
	...
	icp_hw_mgr.hfi_mem.ipc_hwmutex.len = len;
	icp_hw_mgr.hfi_mem.ipc_hwmutex.iova = iova;
	icp_hw_mgr.hfi_mem.ipc_hwmutex.smmu_hdl = icp_hw_mgr.iommu_hdl;
...
static int cam_icp_allocate_mem_for_fence_signaling(void)
{
	...
	rc = cam_icp_allocate_device_synx_hwmutex_mem();
	if (rc)
		goto unmap_global_sync;

	rc = cam_icp_allocate_device_ipc_hwmutex_mem();
	if (rc)
...
static int cam_icp_allocate_hfi_mem(void)
{
	...
	if (!rc) {
		if (icp_hw_mgr.fw_based_sys_caching) {
			rc = cam_icp_allocate_llcc_register_mem(&icp_hw_mgr);
			if (rc) {
				CAM_ERR(CAM_ICP, "Unable to allocate llcc memory rc %d", rc);
				if (fwuncached_region_exists) {
					cam_mem_mgr_free_memory_region(
						&icp_hw_mgr.hfi_mem.fw_uncached_generic);
					goto qtbl_alloc_failed;
	...
	struct cam_icp_hw_mgr *hw_mgr)
{
	...
	if (hw_mgr->synx_signaling_en) {
		...
		region_info->region_info_flex[region_info->num_valid_regions].region_type =
			HFI_MEM_REGION_TYPE_DEVICE;
		region_info->region_info_flex[region_info->num_valid_regions].start_addr =
			hw_mgr->hfi_mem.soc_hw_version.iova;
		region_info->region_info_flex[region_info->num_valid_regions].size =
			hw_mgr->hfi_mem.soc_hw_version.len;
	...
	int32_t index)
{
	...
	if (prepare_args->num_in_map_entries > 1) {
		...
		CAM_DBG(CAM_ICP, "No input fences");
		prepare_args->num_in_map_entries = 0;
		ctx_data->hfi_frame_process.in_resource[index] = 0;
	}
	...
	struct cam_synx_core_control *synx_core_ctrl)
{
	...
	if (rc)
		CAM_ERR(CAM_ICP, "Failed to process core control resume: %s",
			CAM_BOOL_TO_YESNO(synx_core_ctrl->core_control));
	...
	struct cam_synx_test_cmd *synx_test_params)
{
	...
	if (!hw_mgr->icp_resumed) {
		...
		/* Set FW log level for synx */
		if (hw_mgr->icp_debug_type)
			hfi_set_debug_level(hw_mgr->icp_debug_type, hw_mgr->icp_dbg_lvl);
	}
	...
	reinit_completion(&hw_mgr->icp_complete);
	rc = hfi_write_cmd(&synx_test_cmd);
	if (rc)
		goto end;

	rem_jiffies = cam_common_wait_for_completion_timeout(
			&hw_mgr->icp_complete, msecs_to_jiffies(timeout));

	if (!rem_jiffies) {
		CAM_ERR(CAM_ICP, "FW response timeout for synx test cmd");
		rc = -ETIMEDOUT;
		goto end;
	}
...
static int cam_icp_mgr_alloc_devs(struct device_node *np)
{
	...
	icp_hw_mgr.fw_based_sys_caching = cam_cpas_is_fw_based_sys_caching_supported();
